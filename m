...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 4C2272D33EF
	for <lists+linux-scsi@lfdr.de>; Tue,  8 Dec 2020 21:28:45 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1731161AbgLHUQE (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Tue, 8 Dec 2020 15:16:04 -0500
Received: from esa1.microchip.iphmx.com ([68.232.147.91]:21474 "EHLO
        esa1.microchip.iphmx.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1731135AbgLHUOr (ORCPT
        <rfc822;linux-scsi@vger.kernel.org>); Tue, 8 Dec 2020 15:14:47 -0500
DKIM-Signature: v=1; a=rsa-sha256; c=simple/simple;
  d=microchip.com; i=@microchip.com; q=dns/txt; s=mchp;
  t=1607458486; x=1638994486;
  h=subject:from:to:cc:date:message-id:in-reply-to:
   references:mime-version:content-transfer-encoding;
  bh=RnkZzC6TwhnH+mhfekNJ2Ln8Dit1FDFWZK1UAZb4F2E=;
  b=KfUkcNirqow0jcqkhMi7fGUf7aduIA/NM/fbjMh5RfJ6BuZMektdY5ro
   uKen06dzlczBpKt5VCUJDTujAkVH6vkyfbKgOVLLnwIorKFAPW5X2IAm+
   mCiib0G0j3+5E3giRvxAS1Xc7J28CtGZnb9LRso3FfZiVL5aSVISN6IZy
   5hwiTNWkPJcpSmzwBUUTa6du9adJU/oFmifgYfUQ1s4+kFMsc+GC8vLN7
   5ofJ2nmczbYKYFS04eiWcpWfdbq2OzsSewK12BbUF0KiyLJbK4+oO4uLC
   Xaq+1g04KQ1JGfICeTRRU5yUPGJe6dUoX9ZjKoiGnvk9kFVCsDjLJtRKP
   w==;
IronPort-SDR: 
X-IronPort-AV: E=Sophos;i="5.78,403,1599548400"; 
   d="scan'208";a="106782220"
Received: from smtpout.microchip.com (HELO email.microchip.com) ([198.175.253.82])
  by esa1.microchip.iphmx.com with ESMTP/TLS/AES256-SHA256; 08 Dec 2020 12:36:59 -0700
Received: from chn-vm-ex03.mchp-main.com (10.10.85.151) by
 chn-vm-ex01.mchp-main.com (10.10.85.143) with Microsoft SMTP Server
 (version=TLS1_2, cipher=TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256) id
 15.1.1979.3; Tue, 8 Dec 2020 12:36:58 -0700
Received: from [127.0.1.1] (10.10.115.15) by chn-vm-ex03.mchp-main.com
 (10.10.85.151) with Microsoft SMTP Server id 15.1.1979.3 via Frontend
 Transport; Tue, 8 Dec 2020 12:36:58 -0700
Subject: [PATCH V2 04/25] smartpqi: add support for raid5 and raid6 writes
...
Date:   Tue, 8 Dec 2020 13:36:58 -0600
Message-ID: <160745621830.13450.13731287908882422804.stgit@brunhilda>
...
index 6bcb037ae9d7..ae43fd23c158 100644
...
+		if (rmd->is_write && !ctrl_info->enable_r6_writes)
 		...
 		break;
 	case SA_RAID_ADM:
@@ -2526,6 +2532,20 @@ static int pqi_calc_aio_r5_or_r6(struct pqi_scsi_dev_raid_map_data *rmd,
...
@@ -2567,7 +2587,7 @@ static int pqi_raid_bypass_submit_scsi_cmd(struct pqi_ctrl_info *ctrl_info,
...
@@ -2587,7 +2607,8 @@ static int pqi_raid_bypass_submit_scsi_cmd(struct pqi_ctrl_info *ctrl_info,
...
+		(rmd.layout_map_count > 1 || rmd.is_write)) {
 		...
 		if (rc)
 			return PQI_RAID_BYPASS_INELIGIBLE;
@@ -2622,9 +2643,27 @@ static int pqi_raid_bypass_submit_scsi_cmd(struct pqi_ctrl_info *ctrl_info,
...
@@ -4844,6 +4883,12 @@ static void pqi_calculate_queue_resources(struct pqi_ctrl_info *ctrl_info)
...
@@ -4931,6 +4976,44 @@ static int pqi_build_raid_sg_list(struct pqi_ctrl_info *ctrl_info,
 	return 0;
 }
...
+
 static int pqi_build_aio_sg_list(struct pqi_ctrl_info *ctrl_info,
 	...
 	struct pqi_io_request *io_request)
@@ -5335,6 +5418,88 @@ static int pqi_aio_submit_io(struct pqi_ctrl_info *ctrl_info,
 	return 0;
 }
...
+
 ...
 {
@@ -6298,6 +6463,60 @@ static ssize_t pqi_lockup_action_store(struct device *dev,
 	return -EINVAL;
 }
...
@@ -6306,6 +6525,10 @@ static DEVICE_ATTR(vendor, 0444, pqi_vendor_show, NULL);
...
@@ -6315,6 +6538,8 @@ static struct device_attribute *pqi_shost_attrs[] = {
 	&dev_attr_vendor,
 	&dev_attr_rescan,
