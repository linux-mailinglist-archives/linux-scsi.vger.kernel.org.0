...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id DC3CD2B3B8E
	for <lists+linux-scsi@lfdr.de>; Mon, 16 Nov 2020 03:56:03 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726540AbgKPCzo (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 15 Nov 2020 21:55:44 -0500
Received: from smtp.infotech.no ([82.134.31.41]:49303 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726534AbgKPCzo (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 15 Nov 2020 21:55:44 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 698FE204255;
        Mon, 16 Nov 2020 03:55:39 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id pJZIISEooJnS; Mon, 16 Nov 2020 03:55:32 +0100 (CET)
Received: from xtwo70.bingwo.ca (host-104-157-204-209.dyn.295.ca [104.157.204.209])
        by smtp.infotech.no (Postfix) with ESMTPA id EA3492041C0;
        Mon, 16 Nov 2020 03:55:31 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        kbuild test robot <lkp@intel.com>
Subject: [PATCH v12 25/44] sg: replace rq array with xarray
Date:   Sun, 15 Nov 2020 21:55:28 -0500
Message-Id: <20201116025528.19801-2-dgilbert@interlog.com>
X-Mailer: git-send-email 2.25.1
In-Reply-To: <20201116025528.19801-1-dgilbert@interlog.com>
References: <20201116025528.19801-1-dgilbert@interlog.com>
MIME-Version: 1.0
...
---
 ...
 1 file changed, 997 insertions(+), 563 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index a2888f503fd0..cf156ce4b278 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
@@ -143,36 +143,51 @@ static struct class_interface sg_interface = {
...
@@ -181,7 +196,7 @@ struct sg_request {	/* SG_MAX_QUEUE requests outstanding per file */
...
@@ -189,15 +204,12 @@ struct sg_fd {		/* holds the state of a file descriptor */
...
@@ -220,8 +232,8 @@ struct sg_device { /* holds the state of each scsi generic device */
...
@@ -229,31 +241,32 @@ struct sg_comm_wr_t {  /* arguments to sg_common_write() */
...
@@ -521,6 +534,7 @@ sg_release(struct inode *inode, struct file *filp)
 static ssize_t
...
@@ -543,36 +557,61 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -589,12 +628,10 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
+	h3p->interface_id = '\0';/* indicate v1 or v2 interface (tunnelled) */
 	...
 	h3p->iovec_count = 0;
 	h3p->mx_sb_len = 0;
@@ -615,10 +652,9 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -632,23 +668,23 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -673,61 +709,40 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 	return 0;
 }
...
@@ -845,75 +860,79 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
...
@@ -994,9 +1023,23 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	}
...
+		err = err ? err : err2;
 err_out:
 	sg_finish_scsi_blk_rq(srp);
 	sg_deact_request(sfp, srp);
@@ -1013,26 +1056,27 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
 {
 	int res = 0;
...
@@ -1056,7 +1100,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 		h2p->result = EIO;
 		break;
 	case DID_ERROR:
...
+		h2p->result = (status_byte(rq_result) == GOOD) ? 0 : EIO;
 	...
 	default:
 		h2p->result = EIO;
@@ -1077,6 +1121,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	} else {
...
+	atomic_set(&srp->rq_st, SG_RS_RCV_DONE);
 	...
 	sg_deact_request(sfp, srp);
 	return res;
@@ -1120,13 +1165,13 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1147,20 +1192,20 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
 	}
...
@@ -1231,7 +1276,7 @@ sg_calc_rq_dur(const struct sg_request *srp)
...
+	return res;
 ...
 #ifdef CONFIG_COMPAT
@@ -1365,26 +1516,43 @@ static int put_compat_request_table(struct compat_sg_req_info __user *o,
 }
 #endif
...
@@ -1451,18 +1619,15 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
@@ -1473,44 +1638,48 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		       sdp->max_sgat_sz);
...
@@ -1565,6 +1734,8 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return put_user(val, ip);
...
@@ -1575,7 +1746,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		result = get_user(val, ip);
 		if (result)
 			return result;
...
+		assign_bit(SG_FDEV_LOG_SENSE, sdp->fdev_bm, !!val);
 		...
 		return 0;
@@ -1620,7 +1791,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		break;
 	}
...
@@ -1683,7 +1854,7 @@ sg_poll(struct file *filp, poll_table * wait)
...
+	else if (likely(test_bit(SG_FFD_CMD_Q, sfp->ffd_bm)))
 		...
 		p_res |= EPOLLOUT | EPOLLWRNORM;
@@ -1705,9 +1876,10 @@ static vm_fault_t
...
@@ -1726,12 +1898,18 @@ sg_vma_fault(struct vm_fault *vmf)
...
@@ -1744,6 +1922,7 @@ sg_vma_fault(struct vm_fault *vmf)
 			struct page *pp;
...
+			xa_unlock_irqrestore(&sfp->srp_arr, iflags);
 			...
 			vmf->page = page;
@@ -1752,6 +1931,8 @@ sg_vma_fault(struct vm_fault *vmf)
 		sa += len;
 		offset -= len;
 	}
+out_err_unlock:
...
@@ -1766,9 +1947,10 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 {
 	int k, length;
 	int ret = 0;
...
+	struct sg_request *srp;
 	...
 	if (!filp || !vma)
 		return -ENXIO;
@@ -1784,11 +1966,13 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	rsv_schp = &srp->sgat_h;
 	if (req_sz > (unsigned long)rsv_schp->buflen) {
 		ret = -ENOMEM;
 		goto out;
@@ -1801,11 +1985,12 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 		sa += len;
 	}
...
@@ -1827,8 +2012,10 @@ sg_rq_end_io_usercontext(struct work_struct *work)
 		return;
 	}
 	SG_LOG(3, sfp, "%s: srp=0x%pK\n", __func__, srp);
-	sg_finish_scsi_blk_rq(srp);
...
@@ -1866,85 +2053,100 @@ sg_check_sense(struct sg_device *sdp, struct sg_request *srp, int sense_len)
 }
...
@@ -1972,9 +2174,9 @@ static struct sg_device *
...
+	unsigned long iflags;
 	...
 	if (!sdp)
@@ -1992,7 +2194,7 @@ sg_add_device_helper(struct gendisk *disk, struct scsi_device *scsidp)
 			error = -ENODEV;
 		} else {
...
@@ -2248,31 +2465,40 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	q = sdp->device->request_queue;
 	...
 	/*
 	 * NOTE
@@ -2285,125 +2511,148 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
@@ -2412,6 +2661,7 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
+	struct sg_scatter_hold *schp = &srp->sgat_h;
 	...
 	if (unlikely(minlen <= 0)) {
 		if (minlen < 0)
@@ -2422,9 +2672,8 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
@@ -2488,13 +2737,15 @@ sg_remove_sgat_helper(struct sg_fd *sfp, struct sg_scatter_hold *schp)
...
@@ -2509,7 +2760,7 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 {
 	int k, num, res;
 	struct page *pgp;
...
@@ -2523,11 +2774,11 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 			break;
 		}
 		if (num > num_xfer) {
...
@@ -2550,135 +2801,273 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 static struct sg_request *
...
+ */
 ...
 {
@@ -2686,34 +3075,43 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
...
@@ -2727,28 +3125,64 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2779,12 +3213,14 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	struct sg_request *srp;
...
@@ -2792,24 +3228,20 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 			continue;
...
+	xa_unlock_irqrestore(xadp, iflags);
 	if (unlikely(sfp != e_sfp))
 		SG_LOG(1, sfp, "%s: xa_erase() return unexpected\n",
 		       __func__);
@@ -3061,6 +3493,7 @@ sg_proc_seq_show_devhdr(struct seq_file *s, void *v)
...
+	int fd_index;
 ...
 static void *
@@ -3144,11 +3577,10 @@ sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
 static void
...
@@ -3159,51 +3591,53 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 		k++;
