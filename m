...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 5B83738DEAD
	for <lists+linux-scsi@lfdr.de>; Mon, 24 May 2021 03:02:45 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232253AbhEXBEK (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 23 May 2021 21:04:10 -0400
Received: from smtp.infotech.no ([82.134.31.41]:33266 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S232225AbhEXBEC (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 23 May 2021 21:04:02 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 3DA2A2042A0;
        Mon, 24 May 2021 03:02:28 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id cnH5RxMlnsdi; Mon, 24 May 2021 03:02:23 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-45-58-219-4.dyn.295.ca [45.58.219.4])
        by smtp.infotech.no (Postfix) with ESMTPA id 7783E204269;
        Mon, 24 May 2021 03:02:19 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        kbuild test robot <lkp@intel.com>
Subject: [PATCH v19 25/45] sg: replace rq array with xarray
Date:   Sun, 23 May 2021 21:01:27 -0400
Message-Id: <20210524010147.94845-26-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c | 1502 ++++++++++++++++++++++++++++-----------------
 1 file changed, 933 insertions(+), 569 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index c2cee7ff0f62..b9488824e5a4 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -228,31 +240,33 @@ struct sg_comm_wr_t {  /* arguments to sg_common_write() */
...
@@ -670,61 +707,40 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 	return 0;
 }
...
@@ -842,70 +858,68 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
...
@@ -986,9 +1010,23 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	}
...
+		err = err ? err : err2;
 err_out:
 	sg_finish_scsi_blk_rq(srp);
 	sg_deact_request(sfp, srp);
@@ -1005,26 +1043,27 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
 {
 	int res = 0;
...
@@ -1048,7 +1087,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 		h2p->result = EIO;
 		break;
 	case DID_ERROR:
...
+		h2p->result = (status_byte(rq_result) == GOOD) ? 0 : EIO;
 	...
 	default:
 		h2p->result = EIO;
@@ -1069,6 +1108,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	} else {
...
+	atomic_set(&srp->rq_st, SG_RS_RCV_DONE);
 	...
 	sg_deact_request(sfp, srp);
 	return res;
@@ -1112,13 +1152,13 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1139,20 +1179,20 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
 	}
...
@@ -1223,7 +1263,7 @@ sg_calc_rq_dur(const struct sg_request *srp)
...
@@ -1357,26 +1454,43 @@ static int put_compat_request_table(struct compat_sg_req_info __user *o,
 }
 #endif
...
@@ -1443,16 +1557,13 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
@@ -1465,44 +1576,48 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		       sdp->max_sgat_sz);
...
@@ -1557,6 +1672,8 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return put_user(val, ip);
...
@@ -1567,7 +1684,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		result = get_user(val, ip);
 		if (result)
 			return result;
...
+		assign_bit(SG_FDEV_LOG_SENSE, sdp->fdev_bm, !!val);
 		...
 		return 0;
@@ -1612,7 +1729,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		break;
 	}
...
@@ -1675,7 +1792,7 @@ sg_poll(struct file *filp, poll_table * wait)
...
+	else if (likely(test_bit(SG_FFD_CMD_Q, sfp->ffd_bm)))
 		...
 		p_res |= EPOLLOUT | EPOLLWRNORM;
@@ -1697,9 +1814,10 @@ static vm_fault_t
...
@@ -1718,12 +1836,18 @@ sg_vma_fault(struct vm_fault *vmf)
...
@@ -1736,6 +1860,7 @@ sg_vma_fault(struct vm_fault *vmf)
 			struct page *pp;
...
+			xa_unlock_irqrestore(&sfp->srp_arr, iflags);
 			...
 			vmf->page = page;
@@ -1744,6 +1869,8 @@ sg_vma_fault(struct vm_fault *vmf)
 		sa += len;
 		offset -= len;
 	}
+out_err_unlock:
...
@@ -1758,9 +1885,10 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 {
 	int k, length;
 	int ret = 0;
...
+	struct sg_request *srp;
 	...
 	if (!filp || !vma)
 		return -ENXIO;
@@ -1776,11 +1904,13 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	rsv_schp = &srp->sgat_h;
 	if (req_sz > (unsigned long)rsv_schp->buflen) {
 		ret = -ENOMEM;
 		goto out;
@@ -1793,11 +1923,12 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 		sa += len;
 	}
...
@@ -1819,8 +1950,10 @@ sg_rq_end_io_usercontext(struct work_struct *work)
 		return;
 	}
 	SG_LOG(3, sfp, "%s: srp=0x%pK\n", __func__, srp);
-	sg_finish_scsi_blk_rq(srp);
...
@@ -1858,85 +1991,95 @@ sg_check_sense(struct sg_device *sdp, struct sg_request *srp, int sense_len)
 }
...
@@ -1964,9 +2107,9 @@ static struct sg_device *
...
+	unsigned long iflags;
 	...
 	if (!sdp)
@@ -1984,7 +2127,7 @@ sg_add_device_helper(struct gendisk *disk, struct scsi_device *scsidp)
 			error = -ENODEV;
 		} else {
...
@@ -2240,31 +2396,40 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	q = sdp->device->request_queue;
 	...
 	/*
 	 * NOTE
@@ -2277,120 +2442,140 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
-		sg_remove_sgat(sfp, req_schp);
 ...
 static int
@@ -2412,9 +2597,8 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
@@ -2475,13 +2659,15 @@ sg_remove_sgat_helper(struct sg_fd *sfp, struct sg_scatter_hold *schp)
...
@@ -2496,7 +2682,7 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 {
 	int k, num, res;
 	struct page *pgp;
...
@@ -2510,11 +2696,11 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 			break;
 		}
 		if (num > num_xfer) {
...
@@ -2537,135 +2723,273 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 static struct sg_request *
...
+ */
 ...
 {
@@ -2673,34 +2997,43 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
...
@@ -2714,28 +3047,64 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2766,12 +3135,14 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	struct sg_request *srp;
...
@@ -2779,24 +3150,20 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 			continue;
...
+	xa_unlock_irqrestore(xadp, iflags);
 	if (unlikely(sfp != e_sfp))
 		SG_LOG(1, sfp, "%s: xa_erase() return unexpected\n",
 		       __func__);
@@ -3046,6 +3413,7 @@ sg_proc_seq_show_devhdr(struct seq_file *s, void *v)
...
+	int fd_index;
 ...
 static void *
@@ -3129,11 +3497,10 @@ sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
 static void
...
@@ -3144,55 +3511,52 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 		k++;
