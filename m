...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 2D6754202D5
	for <lists+linux-scsi@lfdr.de>; Sun,  3 Oct 2021 18:38:04 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231233AbhJCQju (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 3 Oct 2021 12:39:50 -0400
Received: from smtp.infotech.no ([82.134.31.41]:55314 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231319AbhJCQjn (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 3 Oct 2021 12:39:43 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id C08E32041E3;
        Sun,  3 Oct 2021 18:37:52 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id rpHS8viZW3-v; Sun,  3 Oct 2021 18:37:49 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-91-187-47.dyn.295.ca [23.91.187.47])
        by smtp.infotech.no (Postfix) with ESMTPA id 9831F2041B2;
        Sun,  3 Oct 2021 18:36:59 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Damien.LeMoal@wdc.com, Hannes Reinecke <hare@suse.com>
Subject: [PATCH v21 30/46] sg: expand sg_comm_wr_t
Date:   Sun,  3 Oct 2021 12:31:35 -0400
Message-Id: <20211003163151.585349-31-dgilbert@interlog.com>
...
index d7d52c0de9f9..e9a91b012429 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
@@ -252,36 +252,38 @@ struct sg_device { /* holds the state of each scsi generic device */
...
+	srp = sg_common_write(&cwr);
 	if (IS_ERR(srp))
 		return PTR_ERR(srp);
 	if (o_srp)
@@ -985,13 +981,14 @@ sg_execute_cmd(struct sg_fd *sfp, struct sg_request *srp)
...
+	struct sg_device *sdp = fp->parentdp;
 	...
 	struct sg_io_v4 *h4p;
@@ -1023,40 +1020,36 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 	if (dxfr_len >= SZ_256M)
...
@@ -2672,11 +2665,10 @@ sg_chk_dio_allowed(struct sg_device *sdp, struct sg_request *srp,
 	return false;
 }
...
@@ -2685,8 +2677,7 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+sg_start_req(struct sg_request *srp, struct sg_comm_wr_t *cwrp, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2696,7 +2687,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
 	void __user *up;
 	struct request *rq;
...
@@ -2706,20 +2697,21 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
...
@@ -2758,10 +2750,17 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
 	scsi_rp = scsi_req(rq);
 	srp->rq = rq;
...
@@ -2784,7 +2783,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
 	}
...
@@ -2793,7 +2792,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
...
@@ -3124,7 +3123,7 @@ sg_mk_srp_sgat(struct sg_fd *sfp, bool first, int db_len)
 		return n_srp;
 	if (db_len > 0) {
...
@@ -3177,18 +3176,18 @@ sg_build_reserve(struct sg_fd *sfp, int buflen)
...
+	struct xarray *xafp = &fp->srp_arr;
 ...
 start_again:
@@ -3241,18 +3240,19 @@ sg_setup_req(struct sg_fd *sfp, int dxfr_len, struct sg_comm_wr_t *cwrp)
 		mk_new_srp = true;
 	}
 	if (mk_new_srp) {
...
+		r_srp = sg_mk_srp_sgat(fp, act_empty, dxfr_len);
 		if (IS_ERR(r_srp)) {
 			if (!try_harder && dxfr_len < SG_DEF_SECTOR_SZ &&
 			    num_inactive > 0) {
@@ -3263,11 +3263,11 @@ sg_setup_req(struct sg_fd *sfp, int dxfr_len, struct sg_comm_wr_t *cwrp)
 		}
...
+			SG_LOG(1, fp, "%s: xa_alloc() failed, errno=%d\n",
 			...
 			kfree(r_srp);
@@ -3276,17 +3276,18 @@ sg_setup_req(struct sg_fd *sfp, int dxfr_len, struct sg_comm_wr_t *cwrp)
 		}
 		idx = n_idx;
 		r_srp->rq_idx = idx;
-		r_srp->parentfp = sfp;
...
@@ -3341,6 +3342,8 @@ sg_add_sfp(struct sg_device *sdp)
