...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id D1F526098D8
	for <lists+linux-scsi@lfdr.de>; Mon, 24 Oct 2022 05:26:07 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230424AbiJXD0G (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 23 Oct 2022 23:26:06 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:34710 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230316AbiJXDXA (ORCPT
        <rfc822;linux-scsi@vger.kernel.org>); Sun, 23 Oct 2022 23:23:00 -0400
Received: from smtp.infotech.no (smtp.infotech.no [82.134.31.41])
        by lindbergh.monkeyblade.net (Postfix) with ESMTP id CB15C5DF2C
        for <linux-scsi@vger.kernel.org>; Sun, 23 Oct 2022 20:21:41 -0700 (PDT)
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 34CFF2041D7;
        Mon, 24 Oct 2022 05:21:40 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id wab5CksLaX1B; Mon, 24 Oct 2022 05:21:40 +0200 (CEST)
Received: from treten.bingwo.ca (unknown [10.16.20.11])
        by smtp.infotech.no (Postfix) with ESMTPA id 0729C2041AF;
        Mon, 24 Oct 2022 05:21:38 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        bvanassche@acm.org
Subject: [PATCH v25 26/44] sg: add sg v4 interface support
Date:   Sun, 23 Oct 2022 23:20:40 -0400
Message-Id: <20221024032058.14077-27-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c      | 436 +++++++++++++++++++++++++++++++++--------
 include/uapi/scsi/sg.h |  37 +++-
 2 files changed, 394 insertions(+), 79 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index f46b2e4a2e0f..6cd1a8b77d8f 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -104,6 +107,7 @@ enum sg_rq_state {	/* N.B. sg_rq_state_arr assumes SG_RS_AWAIT_RCV==2 */
...
@@ -167,6 +171,15 @@ struct sg_slice_hdr3 {
 	void __user *usr_ptr;
 };
...
@@ -240,7 +256,10 @@ struct sg_device { /* holds the state of each scsi generic device */
...
@@ -249,13 +268,13 @@ static enum rq_end_io_ret sg_rq_end_io(struct request *rq, blk_status_t status);
...
@@ -263,12 +282,12 @@ static int sg_read_append(struct sg_request *srp, void __user *outp,
...
@@ -857,16 +947,46 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
 	return 0;
 }
...
@@ -903,25 +1053,7 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 		goto err_out;
 	}
...
@@ -933,7 +1065,6 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
-
 ...
 sg_get_ready_srp(struct sg_fd *sfp, struct sg_request **srpp, int pack_id,
 		 bool *busy)
@@ -949,7 +1080,7 @@ sg_get_ready_srp(struct sg_fd *sfp, struct sg_request **srpp, int pack_id,
  * negated errno value.
  */
 static int
...
+sg_copy_sense(struct sg_request *srp, bool v4_active)
 {
 	int sb_len_ret = 0;
 	int scsi_stat;
@@ -959,11 +1090,18 @@ sg_copy_sense(struct sg_request *srp)
...
@@ -980,14 +1118,16 @@ sg_copy_sense(struct sg_request *srp)
 }
...
@@ -1014,7 +1154,7 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 		goto err_out;
 	}
...
@@ -1038,11 +1178,104 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	return err;
 }
...
+
 ...
 sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
@@ -1316,6 +1549,8 @@ sg_fill_request_element(struct sg_fd *sfp, struct sg_request *srp,
...
@@ -1333,7 +1568,7 @@ sg_rq_landed(struct sg_device *sdp, struct sg_request *srp)
  */
 static int
...
@@ -1361,7 +1596,10 @@ sg_wait_event_srp(struct file *filp, struct sg_fd *sfp, void __user *p,
...
@@ -1375,8 +1613,9 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 {
 	int res;
...
@@ -1385,15 +1624,25 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return res;
...
+				   ((u8 __user *)buf) + SZ_SG_IO_HDR,
+				   SZ_SG_IO_V4 - SZ_SG_IO_HDR))
+			return -EFAULT;
+		res = sg_submit_v4(filp, sfp, buf, h4p, true, &srp);
...
+	res = sg_wait_event_srp(filp, sfp, buf, h4p, srp);
 	if (res)
 		SG_LOG(1, sfp, "%s: %s=0x%pK  state: %s\n", __func__,
 		       "unexpected srp", srp,
@@ -1562,6 +1811,12 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	switch (cmd_in) {
 	case SG_IO:
...
@@ -2015,8 +2270,16 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	}
 	...
 	SG_LOG(6, sfp, "%s: pack_id=%d, res=0x%x\n", __func__, srp->pack_id,
 	       srp->rq_result);
@@ -2427,7 +2690,8 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	     struct sg_io_v4 *h4p, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2443,14 +2707,30 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
