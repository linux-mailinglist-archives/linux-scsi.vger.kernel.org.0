...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 155907D7E21
	for <lists+linux-scsi@lfdr.de>; Thu, 26 Oct 2023 10:12:35 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229882AbjJZIMf (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Thu, 26 Oct 2023 04:12:35 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:43970 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229567AbjJZIMe (ORCPT
        <rfc822;linux-scsi@vger.kernel.org>); Thu, 26 Oct 2023 04:12:34 -0400
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 2C1A9B8
        for <linux-scsi@vger.kernel.org>; Thu, 26 Oct 2023 01:12:30 -0700 (PDT)
Received: by smtp.kernel.org (Postfix) with ESMTPS id B0A1DC433C7
        for <linux-scsi@vger.kernel.org>; Thu, 26 Oct 2023 08:12:29 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1698307949;
        bh=R7ZV5Uunf24d4N8ztnomDmFrRRtWDbYHtRaVTtoSXlg=;
        h=From:To:Subject:Date:In-Reply-To:References:From;
        b=
Received: by aws-us-west-2-korg-bugzilla-1.web.codeaurora.org (Postfix, from userid 48)
        id 98E24C53BCD; Thu, 26 Oct 2023 08:12:29 +0000 (UTC)
From:   bugzilla-daemon@kernel.org
To:     linux-scsi@vger.kernel.org
Subject: [Bug 218030] Marvell 88SE6320 SAS controller (mvsas) cannot survive
 ACPI S3 or ACPI S4
Date:   Thu, 26 Oct 2023 08:12:29 +0000
...
Message-ID: <bug-218030-11613-Si6BAoAN7G@https.bugzilla.kernel.org/>
...
(In reply to Nikolaos Barkas from comment #0)
> Hello guys,
>=20
> marvell sas driver fails to resume from suspend/hibernate properly and the
> disks disappear. The issue most likely affects all the chips supported by
> this driver.
>=20
> For the first test, I booted the system with 2 ATA disks attached to the
> onboard SAS controller and did a suspend/resume cycle followed by a get
> health info cmd.
> For the second test, I booted the system with a SAS disk attached to the
> onboard SAS controller and did a hibernate/resume cycle followed by a gno=
me
> disks read cmd.
> The boot drive is not attached to the marvell sas controller.
>=20
> When I try to access the disks after resuming from suspend/hibernate the
> system becomes dead slow and the disks show no sign of life.
> The system cannot power off by itself at this stage, it needs a hard reset
> or a power off by the psu switch.=20
>=20
> The most important parts of the kernel log can be found below.
> I have provided mainly the mvsas messages and those that have to do with =
the
> suspend/resume.
>=20
...
> [    0.345980] pci 0000:06:00.0: reg 0x20: [mem 0xfbaff000-0xfbafffff 64b=
it]
> [    0.345991] pci 0000:06:00.0: reg 0x30: [mem 0xfba80000-0xfbabffff pre=
f]
> [    0.346048] pci 0000:06:00.0: supports D1
> [    0.346049] pci 0000:06:00.0: PME# supported from D0 D1 D3hot
> [    0.346078] pci 0000:06:00.0: 2.000 Gb/s available PCIe bandwidth,
> limited by 2.5 GT/s PCIe x1 link at 0000:00:1c.4 (capable of 8.000 Gb/s w=
ith
...
> [    1.455288] mvsas 0000:06:00.0: mvsas: PCI-E x1, Bandwidth Usage: 2.5 =
Gbps
>=20
> [    4.295662] scsi host0: mvsas
> [    4.297425] sas: phy-0:0 added to port-0:0, phy_mask:0x1
> (0000000000000000)
...
>=20
> [    4.458061] ata9.00: ATA-7: WDC WD740ADFD-00NLR4, 21.07QR4, max UDMA/1=
33
> [    4.458068] ata9.00: 145226112 sectors, multi 0: LBA48 NCQ (depth 32)
> [    4.461181] ata9.00: configured for UDMA/133
> [    4.461304] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 0 tri=
es:
> 1
> [    4.471938] scsi 0:0:0:0: Direct-Access     ATA      WDC WD740ADFD-00
> 7QR4 PQ: 0 ANSI: 5
> [    4.472713] sas: DONE DISCOVERY on port 0, pid:76, result:0
> [    4.472809] sas: phy-0:1 added to port-0:1, phy_mask:0x2
> (0100000000000000)
...
> [    4.601099] scsi 9:0:0:0: Direct-Access     Corsair  Flash Voyager=20=
=20=20
> 1100 PQ: 0 ANSI: 0 CCS
> [    4.640508] ata10.00: ATA-8: WDC WD5002ABYS-01B1B0, 02.03B03, max UDMA=
/133
> [    4.640515] ata10.00: 976773168 sectors, multi 0: LBA48 NCQ (depth 32)
> [    4.641755] ata10.00: configured for UDMA/133
> [    4.641848] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 0 tri=
es:
> 1
> [    4.652603] scsi 0:0:1:0: Direct-Access     ATA      WDC WD5002ABYS-0
> 3B03 PQ: 0 ANSI: 5
...
> [    4.654420] sd 0:0:1:0: [sdc] 976773168 512-byte logical blocks: (500
> GB/466 GiB)
> [    4.654445] sd 0:0:1:0: [sdc] Write Protect is off
> [    4.654448] sd 0:0:1:0: [sdc] Mode Sense: 00 3a 00 00
> [    4.654454] sd 0:0:0:0: [sdb] Write cache: enabled, read cache: enable=
d,
> doesn't support DPO or FUA
> [    4.654484] sd 0:0:1:0: [sdc] Write cache: enabled, read cache: enable=
d,
> doesn't support DPO or FUA
...
> [   13.853737] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 1 tri=
es:
> 1
...
> [   14.031906] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 1 tri=
es:
> 1
>=20
...
> [   15.007777] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 1 tri=
es:
> 1
>=20
...
> [  252.047938] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 1 tri=
es:
> 1
...
> [  252.287846] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 1 tri=
es:
> 1
>=20
...
> [  441.650468] RTL8211B Gigabit Ethernet r8169-0-800:00: attached PHY dri=
ver
> (mii_bus:phy_addr=3Dr8169-0-800:00, irq=3DMAC)
> [  441.715440] r8169 0000:08:00.0 enp8s0: Link is Down
> [  441.716682] RTL8211B Gigabit Ethernet r8169-0-900:00: attached PHY dri=
ver
> (mii_bus:phy_addr=3Dr8169-0-900:00, irq=3DMAC)
> [  441.780180] r8169 0000:09:00.0 enp9s0: Link is Down
> [  472.289330] usb 6-1: reset high-speed USB device number 2 using ehci-p=
ci
>=20
...
> [  610.481292]        (detected by 5, t=3D5252 jiffies, g=3D53581, q=3D31=
630
> ncpus=3D12)
...
> [  610.481312] CPU: 11 PID: 3152 Comm: kworker/u32:59 Tainted: G         =
 I=20
> 6.1.57-vanilla #14
> [  610.481318] Hardware name: System manufacturer System Product Name/P6T=
 WS
...
> ff 03 00 00 41 8d 56 01 8b 1c 90 49 89 d4 41 89 df 41 81 e7 00 00 08 00 <=
f7>
> c3 00 00 01 00 74 58 31 d2 89 de 48 89 ef e8 0b f9 ff ff 45 85
...
> [  665.286200] Modules linked in: nls_ascii nls_cp437 vfat fat nvme_fabri=
cs
...
> coretemp acpi_cpufreq hwmon_vid msr parport_pc ppdev lp parport fuse
> efi_pstore loop configfs ip_tables x_tables autofs4 ext4 crc32c_generic
> crc16 mbcache jbd2 dm_crypt dm_mod hid_logitech_hidpp hid_logitech_dj sd_=
mod
> t10_pi crc64_rocksoft_generic crc64_rocksoft hid_generic crc_t10dif uas
> ata_generic crct10dif_generic usbhid hid usb_storage crc64 pata_marvell
> nouveau crct10dif_pclmul crct10dif_common
...
> drm_ttm_helper mvsas ttm libsas ahci scsi_transport_sas drm_kms_helper r8=
169
> libahci ehci_pci uhci_hcd realtek firewire_ohci ehci_hcd mxm_wmi aesni_in=
tel
> libata crypto_simd lpc_ich cryptd psmouse drm i2c_i801 mdio_devres scsi_m=
od
> usbcore i2c_smbus firewire_core libphy crc_itu_t scsi_common usb_common w=
mi
> button
> [  665.286257] CPU: 11 PID: 3152 Comm: kworker/u32:59 Tainted: G         =
 I=20
> 6.1.57-vanilla #14
> [  665.286259] Hardware name: System manufacturer System Product Name/P6T=
 WS
> PRO, BIOS 1205    09/24/2010
...
> e8 31 e4 af c6 89 d8 5b c3 cc cc cc cc 66 66 2e 0f 1f 84 00 00 00 00 00 <=
0f>
> 1f 44 00 00 41 57 41 56 41 55 41 89 f5 41 81 e5 ff 0f 00 00 41
...
> [  673.501286]        (detected by 0, t=3D21007 jiffies, g=3D53581, q=3D1=
20696
> ncpus=3D12)
...
> [  673.501298] CPU: 11 PID: 3152 Comm: kworker/u32:59 Tainted: G         =
 I=20
> 6.1.57-vanilla #14
> [  673.501301] Hardware name: System manufacturer System Product Name/P6T=
 WS
> PRO, BIOS 1205    09/24/2010
...
> e5 ff 0f 00 00 41 54 45 89 ec 55 4b 8d 04 a4 53 49 8d 04 44 48 c1 e0 03 <=
4c>
> 8d 0c 07 48 83 ec 20 49 8b 99 08 fd 00 00 89 54 24 08 4c 89 0c
...
> [  331.847662] Freezing user space processes completed (elapsed 0.001
> seconds)
...
> [  335.133826] PM: hibernation: Normal pages needed: 480821 + 1024,
> available pages: 1613904
...
> [  335.297280] usb 3-4: reset high-speed USB device number 2 using ehci-p=
ci
...
> [  335.769243] usb 3-4.3: reset full-speed USB device number 3 using ehci=
-pci
> [  335.957245] usb 3-4.4: reset full-speed USB device number 4 using ehci=
-pci
...
> [  336.636953] RTL8211B Gigabit Ethernet r8169-0-800:00: attached PHY dri=
ver
> (mii_bus:phy_addr=3Dr8169-0-800:00, irq=3DMAC)
> [  336.703336] r8169 0000:08:00.0 enp8s0: Link is Down
> [  336.704991] RTL8211B Gigabit Ethernet r8169-0-900:00: attached PHY dri=
ver
> (mii_bus:phy_addr=3Dr8169-0-900:00, irq=3DMAC)
...
> [  463.977425] sas: sas_eh_handle_sas_errors: task 0x00000000f13c22cf is =
done
...
> [  472.026044] sas: sas_eh_handle_sas_errors: task 0x0000000089d39e25 is =
done
...
> [  513.081247] sas: sas_scsi_find_task: task 0x000000002d226601 failed to
> abort
...
> [  515.321243] sas: --- Exit sas_scsi_recover_host: busy: 0 failed: 3 tri=
es:
> 1
> [  515.337273] sd 0:0:0:0: [sdb] tag#8 FAILED Result:
> hostbyte=3DDID_NO_CONNECT driverbyte=3DDRIVER_OK cmd_age=3D81s
> [  515.337281] sd 0:0:0:0: [sdb] tag#8 CDB: Read(10) 28 00 11 77 78 98 00=
 04
> 00 00
> [  515.337283] I/O error, dev sdb, sector 293042328 op 0x0:(READ) flags
> 0x4000 phys_seg 8 prio class 2
> [  515.337299] sd 0:0:0:0: [sdb] tag#9 FAILED Result:
> hostbyte=3DDID_NO_CONNECT driverbyte=3DDRIVER_OK cmd_age=3D81s
> [  515.337303] sd 0:0:0:0: [sdb] tag#9 CDB: Read(10) 28 00 11 77 7c 98 00=
 04
> 00 00
> [  515.337305] I/O error, dev sdb, sector 293043352 op 0x0:(READ) flags
> 0x4000 phys_seg 8 prio class 2
> [  515.337315] sd 0:0:0:0: [sdb] tag#10 FAILED Result:
> hostbyte=3DDID_NO_CONNECT driverbyte=3DDRIVER_OK cmd_age=3D81s
...
Can you reproduce above on latest mainline (currently v6.6-rc7)?
