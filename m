...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 476BE6097B9
	for <lists+linux-scsi@lfdr.de>; Mon, 24 Oct 2022 03:12:09 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S229923AbiJXBMH (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 23 Oct 2022 21:12:07 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:39530 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229803AbiJXBMC (ORCPT
        <rfc822;linux-scsi@vger.kernel.org>); Sun, 23 Oct 2022 21:12:02 -0400
Received: from smtp.infotech.no (smtp.infotech.no [82.134.31.41])
        by lindbergh.monkeyblade.net (Postfix) with ESMTP id 0550E63FDF
        for <linux-scsi@vger.kernel.org>; Sun, 23 Oct 2022 18:11:57 -0700 (PDT)
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 1D3122041AF;
        Mon, 24 Oct 2022 03:02:58 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 4SvycgDw3bnW; Mon, 24 Oct 2022 03:02:54 +0200 (CEST)
Received: from treten.bingwo.ca (host-45-78-203-98.dyn.295.ca [45.78.203.98])
        by smtp.infotech.no (Postfix) with ESMTPA id 72BC02041BB;
        Mon, 24 Oct 2022 03:02:52 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        bvanassche@acm.org
Subject: [PATCH 5/5] scsi_debug: change store from vmalloc to sgl
Date:   Sun, 23 Oct 2022 21:02:44 -0400
Message-Id: <20221024010244.9522-6-dgilbert@interlog.com>
...
@@ -2990,6 +3029,40 @@ static inline int check_device_access_params
 	return 0;
 }
...
@@ -3008,13 +3081,14 @@ static inline struct sdeb_store_info *devip2sip(struct sdebug_dev_info *devip,
...
+	struct scatterlist *store_sgl;
 	...
 	if (do_write) {
 		dir = DMA_TO_DEVICE;
@@ -3027,25 +3101,38 @@ static int do_device_access(struct sdeb_store_info *sip, struct scsi_cmnd *scp,
 		return 0;
...
@@ -3062,37 +3149,66 @@ static int do_dout_fetch(struct scsi_cmnd *scp, u32 num, u8 *doutp)
...
@@ -3185,13 +3301,22 @@ static int prot_verify_read(struct scsi_cmnd *scp, sector_t start_sec,
 {
 	int ret = 0;
 	unsigned int i;
+	const u32 lb_size = sdebug_sector_size;
...
@@ -3205,11 +3330,19 @@ static int prot_verify_read(struct scsi_cmnd *scp, sector_t start_sec,
...
@@ -3217,6 +3350,8 @@ static int prot_verify_read(struct scsi_cmnd *scp, sector_t start_sec,
...
@@ -3431,6 +3566,7 @@ static int prot_verify_write(struct scsi_cmnd *SCpnt, sector_t start_sec,
...
@@ -3480,7 +3616,7 @@ static int prot_verify_write(struct scsi_cmnd *SCpnt, sector_t start_sec,
...
+			dpage_offset += lb_size;
 		...
 		sg_miter_stop(&diter);
@@ -3555,8 +3691,8 @@ static void map_region(struct sdeb_store_info *sip, sector_t lba,
...
@@ -3566,10 +3702,26 @@ static void unmap_region(struct sdeb_store_info *sip, sector_t lba,
 		    index < map_size) {
...
@@ -3727,7 +3879,7 @@ static int resp_write_scat(struct scsi_cmnd *scp,
 	u8 wrprotect;
 	u16 lbdof, num_lrd, k;
...
+	const u32 lb_size = sdebug_sector_size;
 	...
 	u64 lba;
 	int ret, res;
@@ -3885,13 +4037,12 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
...
@@ -3907,15 +4058,17 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
 	}
 	lbaa = lba;
 	block = do_div(lbaa, sdebug_store_sectors);
...
+		ret = do_device_access(sip, scp, 0, lba, 1, true);
 	if (-1 == ret) {
 		...
 		return DID_ERROR << 16;
@@ -3926,9 +4079,11 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
...
@@ -3937,7 +4092,6 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
...
@@ -4043,15 +4197,14 @@ static int resp_write_buffer(struct scsi_cmnd *scp,
 	return 0;
 }
...
+	const u32 lb_size = sdebug_sector_size;
 	...
 	int ret;
 	int retval = 0;
@@ -4074,25 +4227,21 @@ static int resp_comp_write(struct scsi_cmnd *scp,
 	if (ret)
 		return ret;
 	dnum = 2 * num;
...
@@ -4100,7 +4249,6 @@ static int resp_comp_write(struct scsi_cmnd *scp,
 		map_region(sip, lba, num);
 cleanup:
 	sdeb_write_unlock(sip);
-	kfree(arr);
...
@@ -4246,12 +4394,12 @@ static int resp_pre_fetch(struct scsi_cmnd *scp,
 			  struct sdebug_dev_info *devip)
 {
 	int res = 0;
-	u64 lba;
...
@@ -4264,21 +4412,21 @@ static int resp_pre_fetch(struct scsi_cmnd *scp,
...
@@ -4395,7 +4543,7 @@ static int resp_verify(struct scsi_cmnd *scp, struct sdebug_dev_info *devip)
 	u32 vnum, a_num, off;
 	const u32 lb_size = sdebug_sector_size;
 	u64 lba;
-	u8 *arr;
...
@@ -4429,30 +4577,31 @@ static int resp_verify(struct scsi_cmnd *scp, struct sdebug_dev_info *devip)
 	if (ret)
 		return ret;
...
+	if (!comp_write_worker(sip, lba, vnum, arr, scp, NULL)) {
 		...
 		goto cleanup;
@@ -4831,9 +4980,16 @@ static void zbc_rwp_zone(struct sdebug_dev_info *devip,
...
@@ -6051,15 +6207,15 @@ static int scsi_debug_show_info(struct seq_file *m, struct Scsi_Host *host)
...
@@ -7178,7 +7334,8 @@ static void sdebug_erase_store(int idx, struct sdeb_store_info *sip)
 	}
...
@@ -7199,6 +7356,41 @@ static void sdebug_erase_all_stores(bool apart_from_first)
...
@@ -7230,13 +7422,21 @@ static int sdebug_add_store(void)
