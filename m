...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 2B90D1388DF
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:18 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387521AbgALX6R (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:17 -0500
Received: from smtp.infotech.no ([82.134.31.41]:51963 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387511AbgALX6Q (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:16 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id DFDD2204255;
        Mon, 13 Jan 2020 00:58:14 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id ia5UJN6L5K0C; Mon, 13 Jan 2020 00:58:11 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id 2040820426F;
        Mon, 13 Jan 2020 00:57:58 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v6 10/37] sg: improve naming
Date:   Mon, 13 Jan 2020 00:57:28 +0100
Message-Id: <20200112235755.14197-11-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c | 239 ++++++++++++++++++++++++----------------------
 1 file changed, 127 insertions(+), 112 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index bce369b49331..2f8c4eccba7e 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
+		sdp->max_sgat_sz = queue_max_segments(q);
 	...
 	if (IS_ERR(sfp)) {
@@ -480,16 +481,18 @@ sg_release(struct inode *inode, struct file *filp)
...
@@ -499,22 +502,33 @@ sg_write(struct file *filp, const char __user *buf, size_t count, loff_t * ppos)
...
@@ -526,56 +540,57 @@ sg_write(struct file *filp, const char __user *buf, size_t count, loff_t * ppos)
...
@@ -601,7 +616,7 @@ sg_submit(struct sg_fd *sfp, struct file *filp, const char __user *buf,
 {
 	int k;
 	struct sg_request *srp;
-	sg_io_hdr_t *hp;
...
@@ -625,7 +640,7 @@ sg_submit(struct sg_fd *sfp, struct file *filp, const char __user *buf,
 		return -ENOSYS;
 	}
...
@@ -666,7 +681,7 @@ sg_common_write(struct sg_fd *sfp, struct sg_request *srp,
 {
 	int k, at_head;
...
+				struct sg_io_hdr *new_hdr;
 				...
 				if (!new_hdr) {
@@ -1135,7 +1150,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
+		return put_user(sdp->max_sgat_sz, ip);
 	case SG_SET_RESERVED_SIZE:
 		...
 		if (result)
@@ -1145,7 +1160,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 		val = min_t(int, val,
...
+		if (val != sfp->reserve.buflen) {
 			if (sfp->mmap_called ||
 			    sfp->res_in_use) {
 				mutex_unlock(&sfp->f_mutex);
@@ -1158,7 +1173,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 		mutex_unlock(&sfp->f_mutex);
 		return 0;
...
@@ -1341,12 +1356,12 @@ sg_vma_fault(struct vm_fault *vmf)
 	}
...
+	for (k = 0; k < rsv_schp->num_sgat && sa < vma->vm_end; k++) {
 		...
 		if (offset < len) {
@@ -1390,14 +1405,14 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	for (k = 0; k < rsv_schp->num_sgat && sa < vma->vm_end; k++) {
 		...
 		len = (len < length) ? len : length;
 		sa += len;
@@ -1415,7 +1430,8 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 static void
...
@@ -1521,8 +1537,8 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
@@ -1547,7 +1563,7 @@ static struct class *sg_sysfs_class;
...
+sg_add_device_helper(struct gendisk *disk, struct scsi_device *scsidp)
 {
 	...
 	struct sg_device *sdp;
@@ -1589,7 +1605,7 @@ sg_alloc(struct gendisk *disk, struct scsi_device *scsidp)
...
+	sdp->max_sgat_sz = queue_max_segments(q);
 	...
 	kref_init(&sdp->d_ref);
 	error = 0;
@@ -1631,9 +1647,8 @@ sg_add_device(struct device *cl_dev, struct class_interface *cl_intf)
...
@@ -1723,7 +1738,7 @@ sg_remove_device(struct device *cl_dev, struct class_interface *cl_intf)
 					"%s: 0x%p\n", __func__, sdp));
...
@@ -1817,7 +1832,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
 	struct request *rq;
...
@@ -1878,13 +1893,13 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
+			if (dxfer_len > rsv_schp->buflen)
 			...
 			mutex_unlock(&sfp->f_mutex);
 			return res;
@@ -1899,7 +1914,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
@@ -1995,7 +2010,7 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 		  int buff_size)
 {
...
+	int max_sgat_sz = sfp->parentdp->max_sgat_sz;
 	...
 	struct sg_device *sdp = sfp->parentdp;
@@ -2004,13 +2019,13 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 		return -EFAULT;
 	if (0 == blk_size)
...
@@ -2051,9 +2066,9 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 	}		/* end of for loop */
...
+	schp->buflen = blk_size;
 	if (rem_sz > 0)	/* must have failed */
 		return -ENOMEM;
 	return 0;
@@ -2070,12 +2085,12 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 static void
...
@@ -2102,7 +2117,7 @@ sg_rd_append(struct sg_request *srp, void __user *outp, int num_xfer)
 		return 0;
...
+	for (k = 0; k < schp->num_sgat && schp->pages[k]; k++) {
 		if (num > num_xfer) {
 			if (copy_to_user(outp, page_address(schp->pages[k]),
 					   num_xfer))
@@ -2150,20 +2165,20 @@ sg_link_reserve(struct sg_fd *sfp, struct sg_request *srp, int size)
 	rem = size;
...
@@ -2173,10 +2188,10 @@ sg_unlink_reserve(struct sg_fd *sfp, struct sg_request *srp)
 {
...
@@ -2266,7 +2281,7 @@ sg_add_sfp(struct sg_device *sdp)
 		kfree(sfp);
...
@@ -2275,8 +2290,8 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2286,7 +2301,7 @@ sg_add_sfp(struct sg_device *sdp)
 static void
...
@@ -2301,9 +2316,9 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	}
...
@@ -2323,11 +2338,11 @@ sg_remove_sfp(struct kref *kref)
 	unsigned long iflags;
...
+	schedule_work(&sfp->ew_fd.work);
 ...
 static int
@@ -2616,19 +2631,19 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
...
@@ -2652,8 +2667,8 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 					cp = "     ";
 			}
 			seq_puts(s, cp);
...
+			usg = srp->data.num_sgat;
 			seq_puts(s, srp->done ?
 				 ((1 == srp->done) ?  "rcv:" : "fin:")
 				  : "act:");
@@ -2706,8 +2721,8 @@ sg_proc_seq_show_debug(struct seq_file *s, void *v)
 				   scsidp->lun,
