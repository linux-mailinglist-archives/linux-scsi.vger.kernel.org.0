...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id BF3244202C1
	for <lists+linux-scsi@lfdr.de>; Sun,  3 Oct 2021 18:37:02 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231166AbhJCQit (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 3 Oct 2021 12:38:49 -0400
Received: from smtp.infotech.no ([82.134.31.41]:55150 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S230522AbhJCQis (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 3 Oct 2021 12:38:48 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id EC913204245;
        Sun,  3 Oct 2021 18:36:48 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id E04iguj6o9qY; Sun,  3 Oct 2021 18:34:12 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-91-187-47.dyn.295.ca [23.91.187.47])
        by smtp.infotech.no (Postfix) with ESMTPA id 31C39204155;
        Sun,  3 Oct 2021 18:34:03 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Damien.LeMoal@wdc.com
Subject: [PATCH v21 10/46] sg: improve naming
Date:   Sun,  3 Oct 2021 12:31:15 -0400
Message-Id: <20211003163151.585349-11-dgilbert@interlog.com>
...
index 0ade94d5fd91..0ba6cc765149 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -807,7 +822,7 @@ sg_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
 	struct sg_request *srp;
 	int req_pack_id = -1;
 	int ret = 0;
-	sg_io_hdr_t *hp;
+	struct sg_io_hdr *hp;
 	...
 	/*
@@ -837,7 +852,7 @@ sg_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
 		}
 		if (old_hdr->reply_len < 0) {
 			if (count >= SZ_SG_IO_HDR) {
-				sg_io_hdr_t *new_hdr;
+				struct sg_io_hdr *new_hdr;
 				...
 				if (!new_hdr) {
@@ -1134,7 +1149,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
+		return put_user(sdp->max_sgat_sz, ip);
 	case SG_SET_RESERVED_SIZE:
 		...
 		if (result)
@@ -1144,7 +1159,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		val = min_t(int, val,
...
+		if (val != sfp->reserve.buflen) {
 			if (sfp->mmap_called ||
 			    sfp->res_in_use) {
 				mutex_unlock(&sfp->f_mutex);
@@ -1157,7 +1172,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		mutex_unlock(&sfp->f_mutex);
 		return 0;
...
@@ -1333,12 +1348,12 @@ sg_vma_fault(struct vm_fault *vmf)
 	}
...
+	for (k = 0; k < rsv_schp->num_sgat && sa < vma->vm_end; k++) {
 		...
 		if (offset < len) {
@@ -1382,14 +1397,14 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	for (k = 0; k < rsv_schp->num_sgat && sa < vma->vm_end; k++) {
 		...
 		len = (len < length) ? len : length;
 		sa += len;
@@ -1407,7 +1422,8 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 static void
...
@@ -1513,8 +1529,8 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
@@ -1537,7 +1553,7 @@ static struct class *sg_sysfs_class;
...
+sg_add_device_helper(struct scsi_device *scsidp)
 {
 	...
 	struct sg_device *sdp;
@@ -1577,7 +1593,7 @@ sg_alloc(struct scsi_device *scsidp)
...
+	sdp->max_sgat_sz = queue_max_segments(q);
 	...
 	kref_init(&sdp->d_ref);
 	error = 0;
@@ -1611,9 +1627,9 @@ sg_add_device(struct device *cl_dev, struct class_interface *cl_intf)
...
@@ -1702,7 +1718,7 @@ sg_remove_device(struct device *cl_dev, struct class_interface *cl_intf)
 					"%s: 0x%p\n", __func__, sdp));
...
@@ -1796,7 +1812,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
 	struct request *rq;
...
@@ -1856,13 +1872,13 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
+			if (dxfer_len > rsv_schp->buflen)
 			...
 			mutex_unlock(&sfp->f_mutex);
 			return res;
@@ -1877,7 +1893,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
@@ -1966,7 +1982,7 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 		  int buff_size)
 {
...
@@ -1974,13 +1990,13 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 		return -EFAULT;
 	if (0 == blk_size)
...
@@ -2018,9 +2034,9 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 	}		/* end of for loop */
...
+	schp->buflen = blk_size;
 	if (rem_sz > 0)	/* must have failed */
 		return -ENOMEM;
 	return 0;
@@ -2037,12 +2053,12 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 static void
...
@@ -2069,7 +2085,7 @@ sg_rd_append(struct sg_request *srp, void __user *outp, int num_xfer)
 		return 0;
...
+	for (k = 0; k < schp->num_sgat && schp->pages[k]; k++) {
 		if (num > num_xfer) {
 			if (copy_to_user(outp, page_address(schp->pages[k]),
 					   num_xfer))
@@ -2117,22 +2133,21 @@ sg_link_reserve(struct sg_fd *sfp, struct sg_request *srp, int size)
 	rem = size;
...
-	}
 ...
 static void
@@ -2140,10 +2155,10 @@ sg_unlink_reserve(struct sg_fd *sfp, struct sg_request *srp)
 {
...
@@ -2154,7 +2169,7 @@ sg_unlink_reserve(struct sg_fd *sfp, struct sg_request *srp)
...
@@ -2233,7 +2248,7 @@ sg_add_sfp(struct sg_device *sdp)
 		kfree(sfp);
...
@@ -2242,8 +2257,8 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2253,7 +2268,7 @@ sg_add_sfp(struct sg_device *sdp)
 static void
...
+	struct sg_fd *sfp = container_of(work, struct sg_fd, ew_fd.work);
 	...
 	struct sg_request *srp;
 	unsigned long iflags;
@@ -2268,9 +2283,9 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	}
...
@@ -2290,11 +2305,11 @@ sg_remove_sfp(struct kref *kref)
 	unsigned long iflags;
...
+	schedule_work(&sfp->ew_fd.work);
 ...
 static int
@@ -2581,22 +2596,22 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
...
@@ -2616,8 +2631,8 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 					cp = "     ";
 			}
 			seq_puts(s, cp);
...
+			usg = srp->data.num_sgat;
 			seq_puts(s, srp->done ?
 				 ((1 == srp->done) ?  "rcv:" : "fin:")
 				  : "act:");
@@ -2670,8 +2685,8 @@ sg_proc_seq_show_debug(struct seq_file *s, void *v)
 				   scsidp->lun,
