...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 6BA341388EE
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:35 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387566AbgALX6f (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:35 -0500
Received: from smtp.infotech.no ([82.134.31.41]:52071 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387556AbgALX6e (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:34 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id AB58620424C;
        Mon, 13 Jan 2020 00:58:32 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id zoedIHYCU+cX; Mon, 13 Jan 2020 00:58:30 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id CCBA42042A6;
        Mon, 13 Jan 2020 00:58:00 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v6 27/37] sg: add sg v4 interface support
Date:   Mon, 13 Jan 2020 00:57:45 +0100
Message-Id: <20200112235755.14197-28-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c      | 456 +++++++++++++++++++++++++++++++++--------
 include/uapi/scsi/sg.h |  37 +++-
 2 files changed, 407 insertions(+), 86 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index 3dc5447b297c..3087a66bb1dd 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
@@ -7,8 +7,9 @@
  *
...
@@ -646,7 +674,7 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
 				     __func__);
 			return -EPERM;
 		}
...
+		res = sg_v3_submit(filp, sfp, h3p, false, NULL);
 ...
 to_v2:
@@ -723,7 +751,7 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
 static inline int
...
@@ -754,8 +782,8 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 }
...
@@ -789,6 +817,67 @@ sg_submit(struct file *filp, struct sg_fd *sfp, struct sg_io_hdr *hp,
 	return 0;
 }
...
@@ -902,16 +991,47 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
 	return 0;
 }
...
@@ -919,12 +1039,32 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 	struct sg_device *sdp = sfp->parentdp;
 	struct sg_request *srp;
...
@@ -934,13 +1074,23 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
+	res = sg_start_req(srp, cwrp->cmnd, cmd_len, h4p, dir);
 	if (res < 0)		/* probably out of space --> -ENOMEM */
 		goto err_out;
 	if (unlikely(SG_IS_DETACHING(sdp))) {
@@ -957,25 +1107,7 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 		goto err_out;
 	}
...
@@ -1007,7 +1139,7 @@ sg_get_ready_srp(struct sg_fd *sfp, struct sg_request **srpp, int pack_id)
  * negated errno value.
  */
 static int
...
+sg_copy_sense(struct sg_request *srp, bool v4_active)
 {
 	int sb_len_ret = 0;
 	int scsi_stat;
@@ -1017,11 +1149,18 @@ sg_copy_sense(struct sg_request *srp)
...
@@ -1038,15 +1177,17 @@ sg_copy_sense(struct sg_request *srp)
 }
...
@@ -1068,7 +1209,7 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, void __user *p)
...
@@ -1092,6 +1233,103 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, void __user *p)
 	return err ? err : 0;
 }
...
+
 ...
 sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	   struct sg_request *srp)
@@ -1370,6 +1608,8 @@ sg_fill_request_element(struct sg_fd *sfp, struct sg_request *srp,
...
@@ -1387,7 +1627,7 @@ sg_rq_landed(struct sg_device *sdp, struct sg_request *srp)
  */
 static int
...
@@ -1415,7 +1655,10 @@ sg_wait_event_srp(struct file *filp, struct sg_fd *sfp, void __user *p,
...
@@ -1429,8 +1672,9 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 {
 	int res;
...
@@ -1439,15 +1683,25 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return res;
...
+	res = sg_wait_event_srp(filp, sfp, p, h4p, srp);
 	if (res) {
 		SG_LOG(1, sfp, "%s: %s=0x%pK  state: %s\n", __func__,
 		       "unexpected srp", srp,
@@ -1672,6 +1926,12 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 	switch (cmd_in) {
 	case SG_IO:
...
@@ -2153,8 +2413,16 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	}
 	...
 	SG_LOG(6, sfp, "%s: pack_id=%d, res=0x%x\n", __func__, srp->pack_id,
 	       srp->rq_result);
@@ -2559,7 +2827,8 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	     struct sg_io_v4 *h4p, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2576,7 +2845,6 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
...
-	struct sg_slice_hdr3 *sh3p = &srp->s_hdr3;
 	...
 	sdp = sfp->parentdp;
@@ -2586,10 +2854,28 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
 			return -ENOMEM;
...
index 595d02b9cfd4..c20f6dde81a9 100644
