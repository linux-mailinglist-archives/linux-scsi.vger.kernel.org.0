...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 70C694202E4
	for <lists+linux-scsi@lfdr.de>; Sun,  3 Oct 2021 18:38:39 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231317AbhJCQk0 (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 3 Oct 2021 12:40:26 -0400
Received: from smtp.infotech.no ([82.134.31.41]:55299 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231215AbhJCQkE (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 3 Oct 2021 12:40:04 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id E27D320414B;
        Sun,  3 Oct 2021 18:38:15 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id RK9ij-t5oMsZ; Sun,  3 Oct 2021 18:38:13 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-91-187-47.dyn.295.ca [23.91.187.47])
        by smtp.infotech.no (Postfix) with ESMTPA id 9CD172041DC;
        Sun,  3 Oct 2021 18:37:51 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Damien.LeMoal@wdc.com
Subject: [PATCH v21 44/46] sg: add blk_poll support
Date:   Sun,  3 Oct 2021 12:31:49 -0400
Message-Id: <20211003163151.585349-45-dgilbert@interlog.com>
...
index 0b1a667f2fbc..ed61d8f8fcba 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
+		}
+	}
+	blk_execute_rq_nowait(NULL, rqq, (int)at_head, sg_rq_end_io);
+	set_bit(SG_FRQ_ISSUED, srp->frq_bm);
 }
 
 /*
@@ -1694,6 +1712,13 @@ sg_wait_event_srp(struct file *filp, struct sg_fd *sfp, void __user *p,
...
@@ -1973,6 +1998,8 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
+			sg_sfp_blk_poll(sfp, 0);	/* LLD may have some ready */
 		...
 		if (val)
 			return put_user(val, ip);
@@ -2157,6 +2184,70 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
@@ -2168,6 +2259,8 @@ sg_poll(struct file *filp, poll_table * wait)
 	__poll_t p_res = 0;
...
@@ -2452,6 +2545,7 @@ sg_rq_end_io(struct request *rqq, blk_status_t status)
 		}
 	}
...
@@ -2477,7 +2571,8 @@ sg_rq_end_io(struct request *rqq, blk_status_t status)
...
@@ -3013,7 +3108,8 @@ sg_finish_scsi_blk_rq(struct sg_request *srp)
...
@@ -3205,6 +3301,8 @@ sg_find_srp_by_id(struct sg_fd *sfp, int pack_id)
...
+		sg_sfp_blk_poll(sfp, 0);	/* LLD may have some ready to push up */
 	if (num_waiting < 1) {
 		...
 		if (num_waiting < 1)
@@ -4013,8 +4111,9 @@ sg_proc_debug_sreq(struct sg_request *srp, int to, char *obp, int len)
...
index 06181c591539..f0e3d274fade 100644
