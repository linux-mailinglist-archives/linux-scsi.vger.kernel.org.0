...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 9FE2D1388E6
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:25 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387540AbgALX6Z (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:25 -0500
Received: from smtp.infotech.no ([82.134.31.41]:51997 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387511AbgALX6Y (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:24 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 35BC9204258;
        Mon, 13 Jan 2020 00:58:23 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 1dQSRFOl6phb; Mon, 13 Jan 2020 00:58:21 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id 7C371204296;
        Mon, 13 Jan 2020 00:57:59 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v6 19/37] sg: introduce request state machine
Date:   Mon, 13 Jan 2020 00:57:37 +0100
Message-Id: <20200112235755.14197-20-dgilbert@interlog.com>
...
index 70d7303f3fd1..7eb77c2247ca 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
@@ -72,7 +72,43 @@ static char *sg_version_date = "20190606";
  */
...
@@ -80,12 +116,11 @@ static char *sg_version_date = "20190606";
...
@@ -129,6 +164,7 @@ struct sg_request {	/* SG_MAX_QUEUE requests outstanding per file */
...
@@ -205,10 +241,15 @@ static void sg_unlink_reserve(struct sg_fd *sfp, struct sg_request *srp);
...
@@ -216,6 +257,8 @@ static void sg_calc_sgat_param(struct sg_device *sdp);
...
@@ -379,15 +422,6 @@ sg_open(struct inode *inode, struct file *filp)
...
-	}
 	...
 	if (op_flags & O_NONBLOCK) {
@@ -492,12 +526,12 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
-	u8 cmnd[SG_MAX_CDB_SIZE];
 	...
 	if (res)
@@ -749,11 +783,25 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 	return 0;
 }
...
@@ -788,6 +836,32 @@ srp_done(struct sg_fd *sfp, struct sg_request *srp)
 	return ret;
 }
...
+
 ...
 {
@@ -1316,7 +1390,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 	case SG_GET_LOW_DMA:
...
+	case SG_NEXT_CMD_LEN:	/* active only in v2 interface */
 		...
 		if (result)
@@ -2242,48 +2316,37 @@ sg_remove_sgat(struct sg_fd *sfp, struct sg_scatter_hold *schp)
 static int
...
+	return res;
 ...
 static void
@@ -2332,6 +2395,22 @@ sg_unlink_reserve(struct sg_fd *sfp, struct sg_request *srp)
 	sfp->res_in_use = 0;
 }
...
@@ -2363,23 +2442,21 @@ sg_add_request(struct sg_fd *sfp)
 	return NULL;
 }
...
-	return res;
 ...
 static struct sg_fd *
@@ -2437,6 +2514,15 @@ sg_add_sfp(struct sg_device *sdp)
 	return sfp;
 }
...
+ */
 ...
 {
@@ -2532,6 +2618,33 @@ sg_get_dev(int dev)
 	return sdp;
 }
...
@@ -2830,8 +2943,9 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
