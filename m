...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id AC8771B3237
	for <lists+linux-scsi@lfdr.de>; Tue, 21 Apr 2020 23:53:46 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726456AbgDUVxn (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Tue, 21 Apr 2020 17:53:43 -0400
Received: from smtp.infotech.no ([82.134.31.41]:43095 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726432AbgDUVxi (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Tue, 21 Apr 2020 17:53:38 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 4661E2041BB;
        Tue, 21 Apr 2020 23:53:36 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id hnNTVhMkdTAF; Tue, 21 Apr 2020 23:53:34 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-251-188-50.dyn.295.ca [23.251.188.50])
        by smtp.infotech.no (Postfix) with ESMTPA id 5F27D204247;
        Tue, 21 Apr 2020 23:53:30 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v9 24/40] sg: xarray for reqs in fd
Date:   Tue, 21 Apr 2020 17:52:42 -0400
Message-Id: <20200421215258.14348-25-dgilbert@interlog.com>
...
index b89bc3b2d7ee..2332e4f498b9 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -759,17 +873,8 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
@@ -778,6 +883,10 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 	else
 		at_head = true;
...
@@ -785,6 +894,22 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
+	return sg_rq_state_chg(srp, old_st, new_st, false, __func__);
 ...
 /*
@@ -1157,12 +1282,9 @@ sg_fill_request_element(struct sg_fd *sfp, struct sg_request *srp,
 static int
...
@@ -1189,15 +1311,12 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
@@ -1254,15 +1373,17 @@ sg_ctl_req_tbl(struct sg_fd *sfp, void __user *p)
 			 GFP_KERNEL);
 	if (!rinfop)
 		return -ENOMEM;
...
@@ -1307,7 +1428,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	int __user *ip = p;
 	struct sg_request *srp;
...
@@ -1330,14 +1451,15 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return 0;
...
@@ -1750,10 +1872,10 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	unsigned long iflags;
 	...
 	if (WARN_ON(srp->done != 0))
 		return;
@@ -1797,7 +1919,6 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
-	spin_lock_irqsave(&sfp->rq_list_lock, iflags);
 	if (unlikely(srp->orphan)) {
 		if (sfp->keep_orphan)
 			srp->sg_io_owned = 0;
@@ -1805,12 +1926,14 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
 			done = 0;
 	}
 	srp->done = done;
-	spin_unlock_irqrestore(&sfp->rq_list_lock, iflags);
...
@@ -2432,20 +2555,19 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
...
@@ -2515,31 +2637,51 @@ sg_build_reserve(struct sg_fd *sfp, int req_size)
 static struct sg_request *
...
-	return NULL;
 ...
 static void
@@ -2549,14 +2691,10 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
...
+	atomic_set(&srp->rq_st, SG_RS_INACTIVE);
 ...
 static struct sg_fd *
@@ -2573,8 +2711,7 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2593,6 +2730,7 @@ sg_add_sfp(struct sg_device *sdp)
 	sfp->parentdp = sdp;
...
@@ -2652,15 +2792,20 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	sdp = sfp->parentdp;
...
@@ -2753,7 +2898,9 @@ sg_rq_st_str(enum sg_rq_state rq_st, bool long_str)
...
@@ -3015,7 +3162,6 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 		if (!fp)
 			continue;
 		k++;
...
@@ -3026,7 +3172,9 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
...
+				continue;
 			...
 			if (srp->res_used) {
@@ -3062,9 +3210,8 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
