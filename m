...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id DE00328EAD5
	for <lists+linux-scsi@lfdr.de>; Thu, 15 Oct 2020 04:07:27 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1732557AbgJOCH1 (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Wed, 14 Oct 2020 22:07:27 -0400
Received: from smtp.infotech.no ([82.134.31.41]:40163 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1729585AbgJOCHN (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Wed, 14 Oct 2020 22:07:13 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 4F95E20423F;
        Thu, 15 Oct 2020 04:07:11 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 4DZ87H8j5eUq; Thu, 15 Oct 2020 04:07:08 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-104-157-204-209.dyn.295.ca [104.157.204.209])
        by smtp.infotech.no (Postfix) with ESMTPA id EDD5B20426C;
        Thu, 15 Oct 2020 04:07:04 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v11 18/44] sg: rework scatter gather handling
Date:   Wed, 14 Oct 2020 22:06:17 -0400
Message-Id: <20201015020643.432908-19-dgilbert@interlog.com>
...
index cd1ae1e38acb..6f7093baa3ff 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
+			res = sg_mk_sgat(req_schp, sfp, dxfer_len);
 			if (res) {
 				mutex_unlock(&sfp->f_mutex);
 				return res;
@@ -2107,117 +2145,104 @@ sg_finish_scsi_blk_rq(struct sg_request *srp)
 	if (srp->res_used)
...
+	memset(schp, 0, sizeof(*schp));         /* zeros buflen and dlen */
 ...
 /*
@@ -2239,12 +2264,12 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 	for (k = 0; k < schp->num_sgat && schp->pages[k]; k++) {
 		if (num > num_xfer) {
...
+					 num))
 			...
 			num_xfer -= num;
 			if (num_xfer <= 0)
@@ -2264,10 +2289,10 @@ sg_build_reserve(struct sg_fd *sfp, int req_size)
 	do {
...
@@ -2371,8 +2396,8 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
 static struct sg_fd *
...
@@ -2389,6 +2414,14 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2405,14 +2438,13 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2443,7 +2475,7 @@ sg_remove_sfp_usercontext(struct work_struct *work)
