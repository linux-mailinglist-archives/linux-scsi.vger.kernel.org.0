...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 47BD0488742
	for <lists+linux-scsi@lfdr.de>; Sun,  9 Jan 2022 02:29:14 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S235016AbiAIB3N (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sat, 8 Jan 2022 20:29:13 -0500
Received: from smtp.infotech.no ([82.134.31.41]:37843 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S233631AbiAIB3K (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sat, 8 Jan 2022 20:29:10 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 5A14A2041CE;
        Sun,  9 Jan 2022 02:29:08 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id yUq6lxjSrIh4; Sun,  9 Jan 2022 02:29:08 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [10.16.20.11])
        by smtp.infotech.no (Postfix) with ESMTPA id 0620420413E;
        Sun,  9 Jan 2022 02:29:06 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        bvanassche@acm.org,
        Shin'ichiro Kawasaki <shinichiro.kawasaki@wdc.com>
Subject: [PATCH v2 8/9] scsi_debug: change store from vmalloc to sgl
Date:   Sat,  8 Jan 2022 20:28:52 -0500
Message-Id: <20220109012853.301953-9-dgilbert@interlog.com>
...
---
 ...
 drivers/scsi/scsi_debug.c | 519 +++++++++++++++++++++++++++-----------
 2 files changed, 371 insertions(+), 151 deletions(-)
...
index 8341258cf561..4f3703830347 100644
...
@@ -1206,6 +1196,45 @@ static int p_fill_from_dev_buffer(struct scsi_cmnd *scp, const void *arr,
 	return 0;
 }
 
+/*
+ * It is important that this function returns (a pointer to) an array of scatterlist objects and
+ * does _not_ use the chain feature. Since large objects are allocated by this driver (e.g.
+ * often > 1 GB) the sg_miter_skip() operation can become expensive since it iterates through
+ * this "list". But because this scatterlist is actually an array, C pointer arithmetic is a
+ * performance win over iteration.
+ */
...
@@ -1300,13 +1329,15 @@ static bool sdeb_sgl_compare_sgl_idx(struct scatterlist *x_sgl, unsigned int x_n
 	return equ;
 }
...
@@ -1347,6 +1378,54 @@ static int fetch_to_dev_buffer(struct scsi_cmnd *scp, unsigned char *arr,
...
@@ -3046,13 +3125,14 @@ static inline struct sdeb_store_info *devip2sip(struct sdebug_dev_info *devip,
...
+	struct scatterlist *store_sgl;
 	...
 	if (do_write) {
 		dir = DMA_TO_DEVICE;
@@ -3065,25 +3145,44 @@ static int do_device_access(struct sdeb_store_info *sip, struct scsi_cmnd *scp,
 		return 0;
...
+	/*
+	 * Reduce number of iterations that sg_miter_skip() needs to do on
+	 * sip (pointer to sdeb_store_info object holding potentially large
+	 * data store). This works because sdeb_sgl_alloc_order() creates a
+	 * C array of scatterlist objects.
+	 */
...
@@ -3100,37 +3199,66 @@ static int do_dout_fetch(struct scsi_cmnd *scp, u32 num, u8 *doutp)
...
+ * false and write the miscompare_idx via miscomp_idxp. This is the COMPARE AND WRITE case.
...
@@ -3221,15 +3349,30 @@ static void dif_copy_prot(struct scsi_cmnd *scp, sector_t sector,
 static int prot_verify_read(struct scsi_cmnd *scp, sector_t start_sec,
 			    unsigned int sectors, u32 ei_lba)
 {
+	bool rd_protect = (scp->cmnd[1] >> 5);  /* RDPROTECT */
 	int ret = 0;
 	unsigned int i;
+	const u32 lb_size = sdebug_sector_size;
 	sector_t sector;
...
+
+	/*
+	 * dif_verify() only needs 1 block of lb_size bytes. If lb_size is
+	 * <= the element size of the scatter-gather list then we don't need
+	 * additional heap storage to "straighten out" the sgl in the worst
+	 * case. Fail if lb_size is too large.
+	 */
+	if (lb_size > (1 << sip->elem_pow2))
+		return -EINVAL;
...
@@ -3242,20 +3385,26 @@ static int prot_verify_read(struct scsi_cmnd *scp, sector_t start_sec,
...
+	return 0;
 ...
 static inline void
@@ -3392,7 +3541,12 @@ static int resp_read_dt0(struct scsi_cmnd *scp, struct sdebug_dev_info *devip)
 
 	/* DIX + T10 DIF */
 	if (unlikely(sdebug_dix && scsi_prot_sg_count(scp))) {
-		switch (prot_verify_read(scp, lba, num, ei_lba)) {
+		ret = prot_verify_read(scp, lba, num, ei_lba);
+		if (ret < 0) {
...
+		switch (ret) {
 		case 1: /* Guard tag error */
 			if (cmd[1] >> 5 != 3) { /* RDPROTECT != 3 */
 				sdeb_read_unlock(sip);
@@ -3449,6 +3603,7 @@ static int prot_verify_write(struct scsi_cmnd *SCpnt, sector_t start_sec,
 			     unsigned int sectors, u32 ei_lba)
 {
 	int ret;
+	const u32 lb_size = sdebug_sector_size;
...
@@ -3498,7 +3653,7 @@ static int prot_verify_write(struct scsi_cmnd *SCpnt, sector_t start_sec,
...
+			dpage_offset += lb_size;
 		...
 		sg_miter_stop(&diter);
@@ -3573,8 +3728,8 @@ static void map_region(struct sdeb_store_info *sip, sector_t lba,
...
@@ -3584,10 +3739,26 @@ static void unmap_region(struct sdeb_store_info *sip, sector_t lba,
 		    index < map_size) {
...
@@ -3745,7 +3916,7 @@ static int resp_write_scat(struct scsi_cmnd *scp,
 	u8 wrprotect;
 	u16 lbdof, num_lrd, k;
...
+	const u32 lb_size = sdebug_sector_size;
 	...
 	u64 lba;
 	int ret, res;
@@ -3903,13 +4074,12 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
...
@@ -3925,15 +4095,17 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
 	}
 	lbaa = lba;
 	block = do_div(lbaa, sdebug_store_sectors);
...
+		ret = do_device_access(sip, scp, 0, lba, 1, true);
 	if (-1 == ret) {
 		...
 		return DID_ERROR << 16;
@@ -3944,9 +4116,11 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
...
@@ -3955,7 +4129,6 @@ static int resp_write_same(struct scsi_cmnd *scp, u64 lba, u32 num,
...
@@ -4061,15 +4234,14 @@ static int resp_write_buffer(struct scsi_cmnd *scp,
 	return 0;
 }
...
+	const u32 lb_size = sdebug_sector_size;
 	...
 	int ret;
 	int retval = 0;
@@ -4092,25 +4264,21 @@ static int resp_comp_write(struct scsi_cmnd *scp,
 	if (ret)
 		return ret;
 	dnum = 2 * num;
...
@@ -4118,7 +4286,6 @@ static int resp_comp_write(struct scsi_cmnd *scp,
 		map_region(sip, lba, num);
 cleanup:
 	sdeb_write_unlock(sip);
-	kfree(arr);
...
@@ -4264,12 +4431,12 @@ static int resp_pre_fetch(struct scsi_cmnd *scp,
 			  struct sdebug_dev_info *devip)
 {
 	int res = 0;
-	u64 lba;
...
@@ -4282,21 +4449,21 @@ static int resp_pre_fetch(struct scsi_cmnd *scp,
...
@@ -4413,7 +4580,7 @@ static int resp_verify(struct scsi_cmnd *scp, struct sdebug_dev_info *devip)
 	u32 vnum, a_num, off;
 	const u32 lb_size = sdebug_sector_size;
 	u64 lba;
-	u8 *arr;
...
@@ -4447,30 +4614,31 @@ static int resp_verify(struct scsi_cmnd *scp, struct sdebug_dev_info *devip)
 	if (ret)
 		return ret;
...
+	if (!comp_write_worker(sip, lba, vnum, arr, scp, NULL)) {
 		...
 		goto cleanup;
@@ -4841,9 +5009,16 @@ static void zbc_rwp_zone(struct sdebug_dev_info *devip,
...
@@ -6106,15 +6281,15 @@ static int scsi_debug_show_info(struct seq_file *m, struct Scsi_Host *host)
...
@@ -7251,7 +7426,8 @@ static void sdebug_erase_store(int idx, struct sdeb_store_info *sip)
 	}
...
@@ -7272,6 +7448,41 @@ static void sdebug_erase_all_stores(bool apart_from_first)
...
@@ -7303,13 +7514,21 @@ static int sdebug_add_store(void)
