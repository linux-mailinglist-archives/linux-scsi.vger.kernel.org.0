...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 4FCD238DEB9
	for <lists+linux-scsi@lfdr.de>; Mon, 24 May 2021 03:02:57 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232307AbhEXBEX (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 23 May 2021 21:04:23 -0400
Received: from smtp.infotech.no ([82.134.31.41]:33309 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S232267AbhEXBEJ (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 23 May 2021 21:04:09 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 6644C204155;
        Mon, 24 May 2021 03:02:38 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 3Jbd1aIoQxBD; Mon, 24 May 2021 03:02:36 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-45-58-219-4.dyn.295.ca [45.58.219.4])
        by smtp.infotech.no (Postfix) with ESMTPA id BF7FF20426F;
        Mon, 24 May 2021 03:02:31 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v19 36/45] sg: rework mmap support
Date:   Sun, 23 May 2021 21:01:38 -0400
Message-Id: <20210524010147.94845-37-dgilbert@interlog.com>
...
index ffcf09440ca6..463a97400f02 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -2177,13 +2180,39 @@ sg_fasync(int fd, struct file *filp, int mode)
...
@@ -2209,7 +2238,7 @@ sg_vma_fault(struct vm_fault *vmf)
...
@@ -2217,44 +2246,37 @@ sg_vma_fault(struct vm_fault *vmf)
 		       offset);
 		goto out_err_unlock;
 	}
-	sa = vma->vm_start;
...
@@ -2265,40 +2287,48 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	return res;
 ...
 static void
@@ -2876,7 +2906,7 @@ sg_start_req(struct sg_request *srp, struct sg_comm_wr_t *cwrp, int dxfer_dir)
 		goto fini;
...
+	us_xfer = !(srp->rq_flags & (SG_FLAG_NO_DXFER | SG_FLAG_MMAP_IO));
 	...
 	rq->end_io_data = srp;
@@ -3005,10 +3035,11 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
 {
...
+	struct page **pgp;
 	...
 	if (unlikely(minlen <= 0)) {
 		if (minlen < 0)
@@ -3023,30 +3054,35 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
@@ -3054,6 +3090,7 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
 		elem_sz >>= 1;
 		goto again;
 	}
+b4_alloc_pages:
 	...
 	schp->pages = NULL;
 	return -ENOMEM;
@@ -3069,7 +3106,7 @@ sg_remove_sgat_helper(struct sg_fd *sfp, struct sg_scatter_hold *schp)
 		return;
...
@@ -3199,7 +3236,7 @@ sg_find_srp_by_id(struct sg_fd *sfp, int pack_id)
 	}
 	return NULL;
 good:
...
@@ -3395,7 +3432,7 @@ sg_setup_req(struct sg_comm_wr_t *cwrp, int dxfr_len)
 		r_srp->parentfp = fp;
...
+	WRITE_ONCE(r_srp->frq_bm[0], cwrp->frq_bm[0]);	/* assumes <= 32 req flags */
 ...
 fini:
@@ -3422,7 +3459,7 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
 		return;
 	sbp = srp->sense_bp;
 	srp->sense_bp = NULL;
-	srp->frq_bm[0] = 0;
