...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 993EF1388F1
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:39 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387570AbgALX6i (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:38 -0500
Received: from smtp.infotech.no ([82.134.31.41]:52055 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387552AbgALX6h (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:37 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 3A75520415B;
        Mon, 13 Jan 2020 00:58:32 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 9ELwebAfN4+U; Mon, 13 Jan 2020 00:58:27 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id 773972042A4;
        Mon, 13 Jan 2020 00:58:00 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v6 25/37] sg: replace rq array with lists
Date:   Mon, 13 Jan 2020 00:57:43 +0100
Message-Id: <20200112235755.14197-26-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c | 1607 +++++++++++++++++++++++++++++----------------
 1 file changed, 1044 insertions(+), 563 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index 6e363d4177fd..1c383ca04877 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
@@ -144,36 +144,51 @@ static struct class_interface sg_interface = {
...
@@ -182,7 +197,7 @@ struct sg_request {	/* SG_MAX_QUEUE requests outstanding per file */
...
@@ -190,15 +205,12 @@ struct sg_fd {		/* holds the state of a file descriptor */
...
@@ -221,8 +233,8 @@ struct sg_device { /* holds the state of each scsi generic device */
...
@@ -230,31 +242,32 @@ struct sg_comm_wr_t {  /* arguments to sg_common_write() */
...
@@ -519,6 +532,39 @@ sg_release(struct inode *inode, struct file *filp)
 	return 0;
 }
 
+/*
+ * Copy of struct in block/scsi_ioctl.c that is used by get_sg_io_hdr() and
+ * put_sg_io_hdr(). Those functions are exported but not the size of this
+ * struct. So it is replicated here so sizeof() will work.
+ */
+#ifdef CONFIG_COMPAT
...
@@ -528,6 +574,7 @@ sg_release(struct inode *inode, struct file *filp)
 static ssize_t
...
@@ -550,36 +597,61 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -596,12 +668,10 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
+	h3p->interface_id = '\0';/* indicate v1 or v2 interface (tunnelled) */
 	...
 	h3p->iovec_count = 0;
 	h3p->mx_sb_len = 0;
@@ -622,7 +692,8 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -637,23 +708,23 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -678,61 +749,40 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 	return 0;
 }
...
@@ -854,73 +904,79 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
...
@@ -974,12 +1035,15 @@ sg_rec_state_v3(struct sg_fd *sfp, struct sg_request *srp)
 {
 	int err = 0;
 	int sb_len_wr;
...
@@ -991,15 +1055,31 @@ sg_rec_state_v3(struct sg_fd *sfp, struct sg_request *srp)
 static int
...
+		err = err ? err : err2;
 	...
 	sg_deact_request(sfp, srp);
 	return err ? err : 0;
@@ -1010,26 +1090,27 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	   struct sg_request *srp)
 {
 	int res = 0;
...
@@ -1053,7 +1134,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 		h2p->result = EIO;
 		break;
 	case DID_ERROR:
...
+		h2p->result = (status_byte(rq_result) == GOOD) ? 0 : EIO;
 	...
 	default:
 		h2p->result = EIO;
@@ -1073,6 +1154,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 		}
 	} else
...
+	atomic_set(&srp->rq_st, SG_RS_RCV_DONE);
 	...
 	sg_deact_request(sfp, srp);
 	return res;
@@ -1116,13 +1198,13 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1143,20 +1225,20 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
 	}
...
+		if (o_srp == cxc_srp) {
+			sfp->rsv_srp = n_srp;
+			__xa_set_mark(xafp, idx, SG_XA_RQ_INACTIVE);
...
+	return res;
 ...
 #ifdef CONFIG_COMPAT
@@ -1364,26 +1552,43 @@ static int put_compat_request_table(struct compat_sg_req_info __user *o,
 }
 #endif
...
@@ -1457,18 +1662,15 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
@@ -1479,44 +1681,48 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 		       sdp->max_sgat_sz);
...
@@ -1571,6 +1777,8 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 		return put_user(val, ip);
...
@@ -1581,7 +1789,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 		result = get_user(val, ip);
 		if (result)
 			return result;
...
+		assign_bit(SG_FDEV_LOG_SENSE, sdp->fdev_bm, !!val);
 		...
 		return 0;
@@ -1628,11 +1836,10 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
@@ -1677,7 +1884,7 @@ sg_poll(struct file *filp, poll_table * wait)
...
+	else if (likely(test_bit(SG_FFD_CMD_Q, sfp->ffd_bm)))
 		...
 		p_res |= EPOLLOUT | EPOLLWRNORM;
@@ -1699,9 +1906,10 @@ static vm_fault_t
...
@@ -1720,12 +1928,18 @@ sg_vma_fault(struct vm_fault *vmf)
...
@@ -1738,6 +1952,7 @@ sg_vma_fault(struct vm_fault *vmf)
 			struct page *pp;
...
+			xa_unlock_irqrestore(&sfp->srp_arr, iflags);
 			...
 			vmf->page = page;
@@ -1746,6 +1961,8 @@ sg_vma_fault(struct vm_fault *vmf)
 		sa += len;
 		offset -= len;
 	}
+out_err_unlock:
...
@@ -1760,9 +1977,10 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 {
 	int k, length;
 	int ret = 0;
...
+	struct sg_request *srp;
 	...
 	if (!filp || !vma)
 		return -ENXIO;
@@ -1778,11 +1996,13 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	rsv_schp = &srp->sgat_h;
 	if (req_sz > (unsigned long)rsv_schp->buflen) {
 		ret = -ENOMEM;
 		goto out;
@@ -1795,15 +2015,23 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 		sa += len;
 	}
...
+ */
 ...
 {
@@ -1821,8 +2049,10 @@ sg_rq_end_io_usercontext(struct work_struct *work)
 		return;
 	}
 	SG_LOG(3, sfp, "%s: srp=0x%pK\n", __func__, srp);
-	sg_finish_scsi_blk_rq(srp);
...
@@ -1860,85 +2090,100 @@ sg_check_sense(struct sg_device *sdp, struct sg_request *srp, int sense_len)
 }
...
@@ -1966,9 +2211,9 @@ static struct sg_device *
...
+	unsigned long iflags;
 	...
 	if (!sdp)
@@ -1986,7 +2231,7 @@ sg_add_device_helper(struct gendisk *disk, struct scsi_device *scsidp)
 			error = -ENODEV;
 		} else {
...
@@ -2242,31 +2502,40 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	q = sdp->device->request_queue;
 	...
 	/*
 	 * NOTE
@@ -2279,132 +2548,157 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
+			res = compat_import_iovec(r0w, up, iov_count,
 						  0, &iov, &i);
 		else
...
+sg_mk_sgat(struct sg_request *srp, struct sg_fd *sfp, int minlen)
 {
 	...
 	int m_size = minlen;
@@ -2415,6 +2709,7 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
+	struct sg_scatter_hold *schp = &srp->sgat_h;
 	...
 	if (unlikely(m_size < 0))
 		return -EFAULT;
@@ -2424,9 +2719,8 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
@@ -2486,12 +2780,15 @@ sg_remove_sgat_helper(struct sg_fd *sfp, struct sg_scatter_hold *schp)
...
@@ -2507,7 +2804,7 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 {
 	int k, num, res;
 	struct page *pgp;
...
@@ -2521,11 +2818,11 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 			break;
 		}
 		if (num > num_xfer) {
...
@@ -2548,135 +2845,274 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 static struct sg_request *
...
+ */
 ...
 {
@@ -2684,34 +3120,43 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
...
@@ -2725,28 +3170,64 @@ sg_add_sfp(struct sg_device *sdp)
...
+		res = __xa_alloc(xafp, &idx, srp, xalrq, GFP_KERNEL);
+		xa_unlock_irqrestore(xafp, iflags);
+		if (res < 0) {
...
@@ -2777,12 +3258,14 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	struct sg_request *srp;
...
@@ -2790,24 +3273,20 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 			continue;
...
+	xa_unlock_irqrestore(xadp, iflags);
 	if (unlikely(sfp != e_sfp))
 		SG_LOG(1, sfp, "%s: xa_erase() return unexpected\n",
 		       __func__);
@@ -3061,6 +3540,7 @@ sg_proc_seq_show_devhdr(struct seq_file *s, void *v)
...
+	int fd_index;
 ...
 static void *
@@ -3144,11 +3624,10 @@ sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
 static void
...
@@ -3159,51 +3638,53 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 		k++;
