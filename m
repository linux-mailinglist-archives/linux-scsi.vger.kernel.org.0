...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 4B82738DEB3
	for <lists+linux-scsi@lfdr.de>; Mon, 24 May 2021 03:02:50 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232266AbhEXBEP (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 23 May 2021 21:04:15 -0400
Received: from smtp.infotech.no ([82.134.31.41]:33289 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S232233AbhEXBEF (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 23 May 2021 21:04:05 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id ADE55204277;
        Mon, 24 May 2021 03:02:32 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id NPkNWtSb7nx8; Mon, 24 May 2021 03:02:29 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-45-58-219-4.dyn.295.ca [45.58.219.4])
        by smtp.infotech.no (Postfix) with ESMTPA id 2B9F520429C;
        Mon, 24 May 2021 03:02:25 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Hannes Reinecke <hare@suse.com>
Subject: [PATCH v19 30/45] sg: expand sg_comm_wr_t
Date:   Sun, 23 May 2021 21:01:32 -0400
Message-Id: <20210524010147.94845-31-dgilbert@interlog.com>
...
index d2805ae84162..b8882ed05487 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -2709,11 +2702,10 @@ sg_chk_dio_allowed(struct sg_device *sdp, struct sg_request *srp,
 	return false;
 }
...
@@ -2722,8 +2714,7 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+sg_start_req(struct sg_request *srp, struct sg_comm_wr_t *cwrp, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2733,7 +2724,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
 	void __user *up;
 	struct request *rq;
...
@@ -2743,20 +2734,21 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
...
@@ -2795,10 +2787,17 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
 	scsi_rp = scsi_req(rq);
 	srp->rq = rq;
...
@@ -2821,7 +2820,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
 	}
...
@@ -2830,7 +2829,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len,
...
@@ -3161,7 +3160,7 @@ sg_mk_srp_sgat(struct sg_fd *sfp, bool first, int db_len)
 		return n_srp;
 	if (db_len > 0) {
...
@@ -3214,18 +3213,18 @@ sg_build_reserve(struct sg_fd *sfp, int buflen)
...
+	struct xarray *xafp = &fp->srp_arr;
 ...
 start_again:
@@ -3278,18 +3277,19 @@ sg_setup_req(struct sg_fd *sfp, int dxfr_len, struct sg_comm_wr_t *cwrp)
 		mk_new_srp = true;
 	}
 	if (mk_new_srp) {
...
+		r_srp = sg_mk_srp_sgat(fp, act_empty, dxfr_len);
 		if (IS_ERR(r_srp)) {
 			if (!try_harder && dxfr_len < SG_DEF_SECTOR_SZ &&
 			    num_inactive > 0) {
@@ -3300,11 +3300,11 @@ sg_setup_req(struct sg_fd *sfp, int dxfr_len, struct sg_comm_wr_t *cwrp)
 		}
...
+			SG_LOG(1, fp, "%s: xa_alloc() failed, errno=%d\n",
 			...
 			kfree(r_srp);
@@ -3313,17 +3313,18 @@ sg_setup_req(struct sg_fd *sfp, int dxfr_len, struct sg_comm_wr_t *cwrp)
 		}
 		idx = n_idx;
 		r_srp->rq_idx = idx;
-		r_srp->parentfp = sfp;
...
@@ -3378,6 +3379,8 @@ sg_add_sfp(struct sg_device *sdp)
