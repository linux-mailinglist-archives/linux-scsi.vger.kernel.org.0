...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 317DC301FFF
	for <lists+linux-scsi@lfdr.de>; Mon, 25 Jan 2021 02:42:29 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726664AbhAYBkj (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 24 Jan 2021 20:40:39 -0500
Received: from smtp.infotech.no ([82.134.31.41]:45836 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726630AbhAYBiu (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 24 Jan 2021 20:38:50 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 0BD522042BE;
        Mon, 25 Jan 2021 02:27:38 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 1hU1oUzES-rB; Mon, 25 Jan 2021 02:27:33 +0100 (CET)
Received: from xtwo70.bingwo.ca (host-104-157-204-209.dyn.295.ca [104.157.204.209])
        by smtp.infotech.no (Postfix) with ESMTPA id 3AC9E20425C;
        Mon, 25 Jan 2021 02:27:23 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        kashyap.desai@broadcom.com, kbuild test robot <lkp@intel.com>
Subject: [PATCH v14 25/45] sg: replace rq array with xarray
Date:   Sun, 24 Jan 2021 20:26:30 -0500
Message-Id: <20210125012650.269411-26-dgilbert@interlog.com>
...
index 01160f3bc587..0c6d25ea33ac 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -519,6 +532,7 @@ sg_release(struct inode *inode, struct file *filp)
 static ssize_t
...
@@ -541,36 +555,61 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -587,12 +626,10 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
+	h3p->interface_id = '\0';/* indicate v1 or v2 interface (tunnelled) */
 	...
 	h3p->iovec_count = 0;
 	h3p->mx_sb_len = 0;
@@ -613,10 +650,9 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -630,23 +666,23 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
...
@@ -671,61 +707,40 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 	return 0;
 }
...
@@ -843,75 +858,79 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
...
@@ -992,9 +1021,23 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	}
...
+		err = err ? err : err2;
 err_out:
 	sg_finish_scsi_blk_rq(srp);
 	sg_deact_request(sfp, srp);
@@ -1011,26 +1054,27 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
 {
 	int res = 0;
...
@@ -1054,7 +1098,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 		h2p->result = EIO;
 		break;
 	case DID_ERROR:
...
+		h2p->result = (status_byte(rq_result) == GOOD) ? 0 : EIO;
 	...
 	default:
 		h2p->result = EIO;
@@ -1075,6 +1119,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	} else {
...
+	atomic_set(&srp->rq_st, SG_RS_RCV_DONE);
 	...
 	sg_deact_request(sfp, srp);
 	return res;
@@ -1118,13 +1163,13 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1145,20 +1190,20 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
 	}
...
@@ -1229,7 +1274,7 @@ sg_calc_rq_dur(const struct sg_request *srp)
...
+	return res;
 ...
 #ifdef CONFIG_COMPAT
@@ -1363,26 +1514,43 @@ static int put_compat_request_table(struct compat_sg_req_info __user *o,
 }
 #endif
...
@@ -1449,18 +1617,15 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
@@ -1471,44 +1636,48 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		       sdp->max_sgat_sz);
...
@@ -1563,6 +1732,8 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return put_user(val, ip);
...
@@ -1573,7 +1744,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		result = get_user(val, ip);
 		if (result)
 			return result;
...
+		assign_bit(SG_FDEV_LOG_SENSE, sdp->fdev_bm, !!val);
 		...
 		return 0;
@@ -1618,7 +1789,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		break;
 	}
...
@@ -1681,7 +1852,7 @@ sg_poll(struct file *filp, poll_table * wait)
...
+	else if (likely(test_bit(SG_FFD_CMD_Q, sfp->ffd_bm)))
 		...
 		p_res |= EPOLLOUT | EPOLLWRNORM;
@@ -1703,9 +1874,10 @@ static vm_fault_t
...
@@ -1724,12 +1896,18 @@ sg_vma_fault(struct vm_fault *vmf)
...
@@ -1742,6 +1920,7 @@ sg_vma_fault(struct vm_fault *vmf)
 			struct page *pp;
...
+			xa_unlock_irqrestore(&sfp->srp_arr, iflags);
 			...
 			vmf->page = page;
@@ -1750,6 +1929,8 @@ sg_vma_fault(struct vm_fault *vmf)
 		sa += len;
 		offset -= len;
 	}
+out_err_unlock:
...
@@ -1764,9 +1945,10 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 {
 	int k, length;
 	int ret = 0;
...
+	struct sg_request *srp;
 	...
 	if (!filp || !vma)
 		return -ENXIO;
@@ -1782,11 +1964,13 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	rsv_schp = &srp->sgat_h;
 	if (req_sz > (unsigned long)rsv_schp->buflen) {
 		ret = -ENOMEM;
 		goto out;
@@ -1799,11 +1983,12 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 		sa += len;
 	}
...
@@ -1825,8 +2010,10 @@ sg_rq_end_io_usercontext(struct work_struct *work)
 		return;
 	}
 	SG_LOG(3, sfp, "%s: srp=0x%pK\n", __func__, srp);
-	sg_finish_scsi_blk_rq(srp);
...
@@ -1864,85 +2051,100 @@ sg_check_sense(struct sg_device *sdp, struct sg_request *srp, int sense_len)
 }
...
@@ -1970,9 +2172,9 @@ static struct sg_device *
...
+	unsigned long iflags;
 	...
 	if (!sdp)
@@ -1990,7 +2192,7 @@ sg_add_device_helper(struct gendisk *disk, struct scsi_device *scsidp)
 			error = -ENODEV;
 		} else {
...
@@ -2246,31 +2463,40 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	q = sdp->device->request_queue;
 	...
 	/*
 	 * NOTE
@@ -2283,125 +2509,148 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
@@ -2410,6 +2659,7 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
+	struct sg_scatter_hold *schp = &srp->sgat_h;
 	...
 	if (unlikely(minlen <= 0)) {
 		if (minlen < 0)
@@ -2420,9 +2670,8 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
@@ -2486,13 +2735,15 @@ sg_remove_sgat_helper(struct sg_fd *sfp, struct sg_scatter_hold *schp)
...
@@ -2507,7 +2758,7 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 {
 	int k, num, res;
 	struct page *pgp;
...
@@ -2521,11 +2772,11 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 			break;
 		}
 		if (num > num_xfer) {
...
@@ -2548,135 +2799,273 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 static struct sg_request *
...
+ */
 ...
 {
@@ -2684,34 +3073,43 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
...
@@ -2725,28 +3123,64 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2777,12 +3211,14 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	struct sg_request *srp;
...
@@ -2790,24 +3226,20 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 			continue;
...
+	xa_unlock_irqrestore(xadp, iflags);
 	if (unlikely(sfp != e_sfp))
 		SG_LOG(1, sfp, "%s: xa_erase() return unexpected\n",
 		       __func__);
@@ -3059,6 +3491,7 @@ sg_proc_seq_show_devhdr(struct seq_file *s, void *v)
...
+	int fd_index;
 ...
 static void *
@@ -3142,11 +3575,10 @@ sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
 static void
...
@@ -3157,51 +3589,53 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 		k++;
