...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id C9F364202C9
	for <lists+linux-scsi@lfdr.de>; Sun,  3 Oct 2021 18:37:37 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231284AbhJCQjX (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 3 Oct 2021 12:39:23 -0400
Received: from smtp.infotech.no ([82.134.31.41]:55216 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231190AbhJCQjW (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 3 Oct 2021 12:39:22 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id EA6A22041BD;
        Sun,  3 Oct 2021 18:37:32 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id wkswx4LfU+W3; Sun,  3 Oct 2021 18:37:30 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-91-187-47.dyn.295.ca [23.91.187.47])
        by smtp.infotech.no (Postfix) with ESMTPA id 7758C2041D4;
        Sun,  3 Oct 2021 18:35:13 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Damien.LeMoal@wdc.com, kernel test robot <lkp@intel.com>,
        Dan Carpenter <dan.carpenter@oracle.com>
Subject: [PATCH v21 18/46] sg: rework scatter gather handling
Date:   Sun,  3 Oct 2021 12:31:23 -0400
Message-Id: <20211003163151.585349-19-dgilbert@interlog.com>
...
index 0eb8625b6fc7..bb2632e46904 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -999,10 +996,43 @@ max_sectors_bytes(struct request_queue *q)
...
+
 ...
 {
@@ -1068,7 +1098,7 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	}
 	srp->orphan = 1;
...
+	return res;
 ...
 static int
@@ -1079,8 +1109,7 @@ sg_set_reserved_sz(struct sg_fd *sfp, int want_rsv_sz)
 		    sfp->res_in_use) {
 			return -EBUSY;
 		}
-
...
+		sg_remove_sgat(sfp, &sfp->reserve);
 	...
 	return 0;
@@ -1524,8 +1553,18 @@ sg_rq_end_io_usercontext(struct work_struct *work)
 {
...
@@ -1655,7 +1694,6 @@ static bool sg_sysfs_valid;
...
-	struct request_queue *q = scsidp->request_queue;
 	...
 	int error;
@@ -1693,7 +1731,7 @@ sg_add_device_helper(struct scsi_device *scsidp)
...
@@ -1983,7 +2021,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
+			res = sg_mk_sgat(req_schp, sfp, dxfer_len);
 			if (res) {
 				mutex_unlock(&sfp->f_mutex);
 				return res;
@@ -2063,113 +2101,100 @@ sg_finish_scsi_blk_rq(struct sg_request *srp)
 	if (srp->res_used)
...
+	memset(schp, 0, sizeof(*schp));		/* zeros buflen and dlen */
 ...
 /*
@@ -2191,12 +2216,12 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 	for (k = 0; k < schp->num_sgat && schp->pages[k]; k++) {
 		if (num > num_xfer) {
...
+					 num))
 			...
 			num_xfer -= num;
 			if (num_xfer <= 0)
@@ -2216,10 +2241,10 @@ sg_build_reserve(struct sg_fd *sfp, int req_size)
 	do {
...
@@ -2323,8 +2348,8 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
 static struct sg_fd *
...
@@ -2341,6 +2366,14 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2357,14 +2390,13 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2395,16 +2427,14 @@ sg_remove_sfp_usercontext(struct work_struct *work)
