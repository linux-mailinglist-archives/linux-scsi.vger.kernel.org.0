...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 18C811388E7
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:26 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387539AbgALX6Y (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:24 -0500
Received: from smtp.infotech.no ([82.134.31.41]:52019 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387533AbgALX6Y (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:24 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id EDF4720424C;
        Mon, 13 Jan 2020 00:58:21 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 4+PLH7NjEz7I; Mon, 13 Jan 2020 00:58:19 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id 56D5E204295;
        Mon, 13 Jan 2020 00:57:59 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v6 18/37] sg: rework scatter gather handling
Date:   Mon, 13 Jan 2020 00:57:36 +0100
Message-Id: <20200112235755.14197-19-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c | 280 +++++++++++++++++++++++++---------------------
 1 file changed, 155 insertions(+), 125 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index debeb91b01d0..70d7303f3fd1 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -994,10 +991,43 @@ max_sectors_bytes(struct request_queue *q)
...
+
 ...
 {
@@ -1063,7 +1093,7 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	}
 	srp->orphan = 1;
...
+	return res;
 ...
 static int
@@ -1074,8 +1104,7 @@ sg_set_reserved_sz(struct sg_fd *sfp, int want_rsv_sz)
 		    sfp->res_in_use) {
 			return -EBUSY;
 		}
-
...
+		sg_remove_sgat(sfp, &sfp->reserve);
 	...
 	return 0;
@@ -1539,8 +1568,18 @@ sg_rq_end_io_usercontext(struct work_struct *work)
 {
...
@@ -1672,7 +1711,6 @@ static bool sg_sysfs_valid;
...
-	struct request_queue *q = scsidp->request_queue;
 	...
 	int error;
@@ -1712,7 +1750,7 @@ sg_add_device_helper(struct gendisk *disk, struct scsi_device *scsidp)
...
@@ -2011,7 +2050,7 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
+			res = sg_mk_sgat(req_schp, sfp, dxfer_len);
 			if (res) {
 				mutex_unlock(&sfp->f_mutex);
 				return res;
@@ -2098,117 +2137,101 @@ sg_finish_scsi_blk_rq(struct sg_request *srp)
 	if (srp->res_used)
...
+	memset(schp, 0, sizeof(*schp));         /* zeros buflen and dlen */
 ...
 /*
@@ -2230,12 +2253,12 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 	for (k = 0; k < schp->num_sgat && schp->pages[k]; k++) {
 		if (num > num_xfer) {
...
+					 num))
 			...
 			num_xfer -= num;
 			if (num_xfer <= 0)
@@ -2255,10 +2278,10 @@ sg_build_reserve(struct sg_fd *sfp, int req_size)
 	do {
...
@@ -2362,8 +2385,8 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
 static struct sg_fd *
...
@@ -2380,6 +2403,14 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2396,14 +2427,13 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2434,7 +2464,7 @@ sg_remove_sfp_usercontext(struct work_struct *work)
