...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 211F13579BC
	for <lists+linux-scsi@lfdr.de>; Thu,  8 Apr 2021 03:46:21 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231281AbhDHBqa (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Wed, 7 Apr 2021 21:46:30 -0400
Received: from smtp.infotech.no ([82.134.31.41]:45204 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231219AbhDHBqZ (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Wed, 7 Apr 2021 21:46:25 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 0633C2041AC;
        Thu,  8 Apr 2021 03:46:13 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id rjK980fHSirh; Thu,  8 Apr 2021 03:46:10 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-45-58-219-4.dyn.295.ca [45.58.219.4])
        by smtp.infotech.no (Postfix) with ESMTPA id 3961E204258;
        Thu,  8 Apr 2021 03:46:06 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v17 27/45] sg: add sg v4 interface support
Date:   Wed,  7 Apr 2021 21:45:13 -0400
Message-Id: <20210408014531.248890-28-dgilbert@interlog.com>
...
index 1cf75aa33dfc..6eebacc58626 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
+	res = sg_wait_event_srp(filp, sfp, p, h4p, srp);
 	if (res)
 		SG_LOG(1, sfp, "%s: %s=0x%pK  state: %s\n", __func__,
 		       "unexpected srp", srp,
@@ -1616,6 +1866,12 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	switch (cmd_in) {
 	case SG_IO:
...
@@ -2098,8 +2354,16 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	}
 	...
 	SG_LOG(6, sfp, "%s: pack_id=%d, res=0x%x\n", __func__, srp->pack_id,
 	       srp->rq_result);
@@ -2504,7 +2768,8 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	     struct sg_io_v4 *h4p, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2521,7 +2786,6 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
...
-	struct sg_slice_hdr3 *sh3p = &srp->s_hdr3;
 	...
 	sdp = sfp->parentdp;
@@ -2531,10 +2795,28 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
 			return -ENOMEM;
...
index f278f60c7802..75017cc49850 100644
