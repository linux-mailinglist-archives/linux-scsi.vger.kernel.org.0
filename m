...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id C2B512F5766
	for <lists+linux-scsi@lfdr.de>; Thu, 14 Jan 2021 04:00:07 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729498AbhAMX0k (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Wed, 13 Jan 2021 18:26:40 -0500
Received: from smtp.infotech.no ([82.134.31.41]:49933 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1729475AbhAMXZa (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Wed, 13 Jan 2021 18:25:30 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 0DF5320426D;
        Wed, 13 Jan 2021 23:46:07 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id QGAmhf02NC0Y; Wed, 13 Jan 2021 23:46:04 +0100 (CET)
Received: from xtwo70.bingwo.ca (host-104-157-204-209.dyn.295.ca [104.157.204.209])
        by smtp.infotech.no (Postfix) with ESMTPA id 274BD204275;
        Wed, 13 Jan 2021 23:46:03 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        kashyap.desai@broadcom.com
Subject: [PATCH v13 27/45] sg: add sg v4 interface support
Date:   Wed, 13 Jan 2021 17:45:08 -0500
Message-Id: <20210113224526.861000-28-dgilbert@interlog.com>
...
index c5a34bb91335..95bfdac68512 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -858,16 +947,47 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
 	return 0;
 }
...
@@ -875,12 +995,32 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 	struct sg_device *sdp = sfp->parentdp;
 	struct sg_request *srp;
...
@@ -890,13 +1030,23 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
@@ -913,25 +1063,7 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 		goto err_out;
 	}
...
@@ -944,7 +1076,6 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
-
 ...
 {
@@ -964,7 +1095,7 @@ sg_get_ready_srp(struct sg_fd *sfp, struct sg_request **srpp, int pack_id)
  * negated errno value.
  */
 static int
...
+sg_copy_sense(struct sg_request *srp, bool v4_active)
 {
 	int sb_len_ret = 0;
 	int scsi_stat;
@@ -974,11 +1105,18 @@ sg_copy_sense(struct sg_request *srp)
...
@@ -995,14 +1133,16 @@ sg_copy_sense(struct sg_request *srp)
 }
...
@@ -1029,7 +1169,7 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 		goto err_out;
 	}
...
@@ -1053,11 +1193,103 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	return err;
 }
...
+
 ...
 sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
@@ -1334,6 +1566,8 @@ sg_fill_request_element(struct sg_fd *sfp, struct sg_request *srp,
...
@@ -1351,7 +1585,7 @@ sg_rq_landed(struct sg_device *sdp, struct sg_request *srp)
  */
 static int
...
@@ -1379,7 +1613,10 @@ sg_wait_event_srp(struct file *filp, struct sg_fd *sfp, void __user *p,
...
@@ -1393,8 +1630,9 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 {
 	int res;
...
@@ -1403,15 +1641,25 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return res;
...
+	res = sg_wait_event_srp(filp, sfp, p, h4p, srp);
 	if (res)
 		SG_LOG(1, sfp, "%s: %s=0x%pK  state: %s\n", __func__,
 		       "unexpected srp", srp,
@@ -1629,6 +1877,12 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	switch (cmd_in) {
 	case SG_IO:
...
@@ -2116,8 +2370,16 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	}
 	...
 	SG_LOG(6, sfp, "%s: pack_id=%d, res=0x%x\n", __func__, srp->pack_id,
 	       srp->rq_result);
@@ -2522,7 +2784,8 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	     struct sg_io_v4 *h4p, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2539,7 +2802,6 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
...
-	struct sg_slice_hdr3 *sh3p = &srp->s_hdr3;
 	...
 	sdp = sfp->parentdp;
@@ -2549,10 +2811,28 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
 			return -ENOMEM;
