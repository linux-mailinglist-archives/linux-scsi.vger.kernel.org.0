...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id DC61A1388DA
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:11 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387509AbgALX6K (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:10 -0500
Received: from smtp.infotech.no ([82.134.31.41]:51956 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387422AbgALX6K (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:10 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id B6C1C204255;
        Mon, 13 Jan 2020 00:58:07 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id dleHzYmQVEke; Mon, 13 Jan 2020 00:58:05 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id 3234820425B;
        Mon, 13 Jan 2020 00:57:57 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Hannes Reinecke <hare@suse.com>
Subject: [PATCH v6 05/37] sg: bitops in sg_device
Date:   Mon, 13 Jan 2020 00:57:23 +0100
Message-Id: <20200112235755.14197-6-dgilbert@interlog.com>
...
index e463ec32d099..4d44a919979d 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -1018,7 +1022,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
+		if (SG_IS_DETACHING(sdp))
 		...
 		if (!scsi_block_when_processing_errors(sdp->device))
 			return -ENXIO;
@@ -1027,8 +1031,8 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 		if (result < 0)
 			return result;
...
+		if (SG_IS_DETACHING(sdp))
 		...
 		if (srp->done) {
@@ -1069,7 +1073,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 		{
 			sg_scsi_id_t v;
...
@@ -1189,18 +1193,18 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 			return result;
 		}
...
@@ -1221,7 +1225,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
+		if (SG_IS_DETACHING(sdp))
 		...
 		break;
 	default:
@@ -1284,7 +1288,7 @@ sg_poll(struct file *filp, poll_table * wait)
 	}
...
+	if (sfp->parentdp && SG_IS_DETACHING(sfp->parentdp)) {
 		...
 		if (count == 0)
@@ -1432,7 +1436,7 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
 		return;
...
+	if (unlikely(SG_IS_DETACHING(sdp)))
 	...
 	sense = req->sense;
@@ -1453,9 +1457,9 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
@@ -1570,7 +1574,7 @@ sg_alloc(struct gendisk *disk, struct scsi_device *scsidp)
...
+	clear_bit(SG_FDEV_DETACHING, sdp->fdev_bm);
 	...
 	sdp->sg_tablesize = queue_max_segments(q);
 	sdp->index = k;
@@ -1695,13 +1699,11 @@ sg_remove_device(struct device *cl_dev, struct class_interface *cl_intf)
...
@@ -2239,7 +2241,7 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2336,8 +2338,8 @@ sg_get_dev(int dev)
...
+		/* If detaching, then the refcount may already be 0, in
 		 ...
 		 */
 		sdp = ERR_PTR(-ENODEV);
@@ -2551,8 +2553,7 @@ sg_proc_seq_show_dev(struct seq_file *s, void *v)
...
+	if (!sdp || !sdp->device || SG_IS_DETACHING(sdp))
 		...
 		scsidp = sdp->device;
@@ -2579,7 +2580,7 @@ sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
 	read_lock_irqsave(&sg_index_lock, iflags);
 	sdp = it ? sg_lookup_dev(it->index) : NULL;
...
@@ -2671,7 +2672,7 @@ sg_proc_seq_show_debug(struct seq_file *s, void *v)
...
@@ -2683,7 +2684,8 @@ sg_proc_seq_show_debug(struct seq_file *s, void *v)
