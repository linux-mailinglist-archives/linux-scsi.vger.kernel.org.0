...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 1F84C17243B
	for <lists+linux-scsi@lfdr.de>; Thu, 27 Feb 2020 17:59:50 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729749AbgB0Q7m (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Thu, 27 Feb 2020 11:59:42 -0500
Received: from smtp.infotech.no ([82.134.31.41]:47185 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1729697AbgB0Q7m (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Thu, 27 Feb 2020 11:59:42 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id C9DF9204199;
        Thu, 27 Feb 2020 17:59:40 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id x3rF2nuOdCYN; Thu, 27 Feb 2020 17:59:39 +0100 (CET)
Received: from xtwo70.bingwo.ca (host-23-251-188-50.dyn.295.ca [23.251.188.50])
        by smtp.infotech.no (Postfix) with ESMTPA id 12A8B20418E;
        Thu, 27 Feb 2020 17:59:32 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v7 20/38] sg: sg_find_srp_by_id
Date:   Thu, 27 Feb 2020 11:58:44 -0500
Message-Id: <20200227165902.11861-21-dgilbert@interlog.com>
...
index b0dc73a6bc20..fb24cbbecda0 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -514,7 +519,6 @@ sg_release(struct inode *inode, struct file *filp)
 static ssize_t
...
+ */
 ...
 {
@@ -798,36 +811,58 @@ sg_rstate_chg(struct sg_request *srp, enum sg_rq_state old_st,
...
@@ -857,12 +892,11 @@ sg_rep_rq_state_fail(struct sg_fd *sfp, enum sg_rq_state exp_old_st,
 #endif
...
@@ -873,27 +907,11 @@ sg_receive_v3(struct sg_fd *sfp, char __user *buf, size_t count,
 		err = -EINVAL;
 		goto err_out;
 	}
-	hp->sb_len_wr = 0;
...
+	err = put_sg_io_hdr(hp, p);
 err_out:
 	...
 	sg_deact_request(sfp, srp);
@@ -974,16 +992,22 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	return res;
 }
...
@@ -998,7 +1022,7 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1017,17 +1041,13 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1035,25 +1055,25 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
 			want_id = h2p->pack_id;
 		}
 	}
...
@@ -1102,6 +1122,52 @@ sg_calc_sgat_param(struct sg_device *sdp)
 	sdp->max_sgat_sz = sz;
 }
...
+
 ...
 {
@@ -1118,6 +1184,7 @@ sg_fill_request_table(struct sg_fd *sfp, struct sg_req_info *rinfo)
...
+		rinfo[val].duration = sg_get_dur(srp, NULL, NULL); /* dummy */
 		if (srp->done)
 			rinfo[val].duration =
 				srp->header.duration;
@@ -1135,6 +1202,18 @@ sg_fill_request_table(struct sg_fd *sfp, struct sg_req_info *rinfo)
 	}
 }
...
@@ -1162,7 +1241,7 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	if (srp->done) {
 		srp->done = 2;
...
+		res = sg_receive_v3(sfp, srp, SZ_SG_IO_HDR, p);
 	...
 	srp->orphan = 1;
@@ -1390,7 +1469,9 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 			return result;
...
@@ -2359,6 +2440,33 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 	return res;
 }
