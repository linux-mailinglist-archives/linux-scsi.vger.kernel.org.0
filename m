...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 7224F17524A
	for <lists+linux-scsi@lfdr.de>; Mon,  2 Mar 2020 04:33:35 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727030AbgCBDde (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 1 Mar 2020 22:33:34 -0500
Received: from smtp.infotech.no ([82.134.31.41]:60605 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726928AbgCBDde (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 1 Mar 2020 22:33:34 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id A1FDF204269;
        Mon,  2 Mar 2020 04:33:30 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id smNjwH9eoibb; Mon,  2 Mar 2020 04:33:28 +0100 (CET)
Received: from xtwo70.bingwo.ca (host-23-251-188-50.dyn.295.ca [23.251.188.50])
        by smtp.infotech.no (Postfix) with ESMTPA id 3C0AC204259;
        Mon,  2 Mar 2020 04:33:23 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v8 27/38] sg: add sg v4 interface support
Date:   Sun,  1 Mar 2020 22:32:38 -0500
Message-Id: <20200302033249.4515-28-dgilbert@interlog.com>
...
---
 ...
 2 files changed, 395 insertions(+), 80 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index 8d9e163ce632..882b5b8877bc 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
+static struct sg_device *sg_get_dev(int min_dev);
 ...
 static struct sg_request *sg_mk_srp_sgat(struct sg_fd *sfp, bool first,
 					 int db_len);
@@ -273,8 +293,11 @@ static const char *sg_rq_st_str(enum sg_rq_state rq_st, bool long_str);
...
@@ -501,8 +526,13 @@ sg_open(struct inode *inode, struct file *filp)
 	goto sg_put;
 }
...
+ */
 ...
 {
@@ -525,8 +555,6 @@ sg_release(struct inode *inode, struct file *filp)
...
@@ -606,7 +634,7 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
 				     __func__);
 			return -EPERM;
 		}
...
+		res = sg_v3_submit(filp, sfp, h3p, false, NULL);
 ...
 to_v2:
@@ -683,7 +711,7 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
 static inline int
...
@@ -714,8 +742,8 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 }
...
@@ -749,6 +777,67 @@ sg_submit(struct file *filp, struct sg_fd *sfp, struct sg_io_hdr *hp,
 	return 0;
 }
...
@@ -857,16 +946,47 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
 	return 0;
 }
...
@@ -874,12 +994,32 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 	struct sg_device *sdp = sfp->parentdp;
 	struct sg_request *srp;
...
@@ -889,13 +1029,23 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
@@ -912,25 +1062,7 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 		goto err_out;
 	}
...
@@ -943,7 +1075,6 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
-
 ...
 {
@@ -963,7 +1094,7 @@ sg_get_ready_srp(struct sg_fd *sfp, struct sg_request **srpp, int pack_id)
  * negated errno value.
  */
 static int
...
+sg_copy_sense(struct sg_request *srp, bool v4_active)
 {
 	int sb_len_ret = 0;
 	int scsi_stat;
@@ -973,11 +1104,18 @@ sg_copy_sense(struct sg_request *srp)
...
@@ -994,14 +1132,16 @@ sg_copy_sense(struct sg_request *srp)
 }
...
@@ -1028,7 +1168,7 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 		goto err_out;
 	}
...
@@ -1052,11 +1192,103 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	return err;
 }
...
+
 ...
 sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
@@ -1333,6 +1565,8 @@ sg_fill_request_element(struct sg_fd *sfp, struct sg_request *srp,
...
@@ -1350,7 +1584,7 @@ sg_rq_landed(struct sg_device *sdp, struct sg_request *srp)
  */
 static int
...
@@ -1378,7 +1612,10 @@ sg_wait_event_srp(struct file *filp, struct sg_fd *sfp, void __user *p,
...
@@ -1392,8 +1629,9 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 {
 	int res;
...
@@ -1402,15 +1640,25 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return res;
...
+	res = sg_wait_event_srp(filp, sfp, p, h4p, srp);
 	if (res)
 		SG_LOG(1, sfp, "%s: %s=0x%pK  state: %s\n", __func__,
 		       "unexpected srp", srp,
@@ -1627,6 +1875,12 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	switch (cmd_in) {
 	case SG_IO:
...
@@ -2121,8 +2375,16 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	}
 	...
 	SG_LOG(6, sfp, "%s: pack_id=%d, res=0x%x\n", __func__, srp->pack_id,
 	       srp->rq_result);
@@ -2527,7 +2789,8 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	     struct sg_io_v4 *h4p, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2544,7 +2807,6 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
...
-	struct sg_slice_hdr3 *sh3p = &srp->s_hdr3;
 	...
 	sdp = sfp->parentdp;
@@ -2554,10 +2816,28 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
 			return -ENOMEM;
...
index 9bd37c6e4f44..51b86d20a7a5 100644
