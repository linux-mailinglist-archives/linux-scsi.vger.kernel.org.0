...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 7D2C01B3249
	for <lists+linux-scsi@lfdr.de>; Tue, 21 Apr 2020 23:54:04 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726508AbgDUVyB (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Tue, 21 Apr 2020 17:54:01 -0400
Received: from smtp.infotech.no ([82.134.31.41]:43140 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726446AbgDUVxt (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Tue, 21 Apr 2020 17:53:49 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 5B36F204255;
        Tue, 21 Apr 2020 23:53:40 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id bvV58sVqJQOk; Tue, 21 Apr 2020 23:53:35 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-251-188-50.dyn.295.ca [23.251.188.50])
        by smtp.infotech.no (Postfix) with ESMTPA id 7546C2041B2;
        Tue, 21 Apr 2020 23:53:31 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v9 25/40] sg: replace rq array with lists
Date:   Tue, 21 Apr 2020 17:52:43 -0400
Message-Id: <20200421215258.14348-26-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c | 1566 +++++++++++++++++++++++++++++----------------
 1 file changed, 1006 insertions(+), 560 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index 2332e4f498b9..c380286ebc4b 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -843,73 +860,79 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
...
@@ -990,9 +1023,23 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	}
...
+		err = err ? err : err2;
 err_out:
 	sg_finish_scsi_blk_rq(srp);
 	sg_deact_request(sfp, srp);
@@ -1009,26 +1056,27 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
 {
 	int res = 0;
...
@@ -1052,7 +1100,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 		h2p->result = EIO;
 		break;
 	case DID_ERROR:
...
+		h2p->result = (status_byte(rq_result) == GOOD) ? 0 : EIO;
 	...
 	default:
 		h2p->result = EIO;
@@ -1073,6 +1121,7 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	} else {
...
+	atomic_set(&srp->rq_st, SG_RS_RCV_DONE);
 	...
 	sg_deact_request(sfp, srp);
 	return res;
@@ -1116,13 +1165,13 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1143,20 +1192,20 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
 	}
...
+	return res;
 ...
 #ifdef CONFIG_COMPAT
@@ -1361,26 +1515,43 @@ static int put_compat_request_table(struct compat_sg_req_info __user *o,
 }
 #endif
...
@@ -1447,18 +1618,15 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
@@ -1469,44 +1637,48 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		       sdp->max_sgat_sz);
...
@@ -1561,6 +1733,8 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return put_user(val, ip);
...
@@ -1571,7 +1745,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		result = get_user(val, ip);
 		if (result)
 			return result;
...
+		assign_bit(SG_FDEV_LOG_SENSE, sdp->fdev_bm, !!val);
 		...
 		return 0;
@@ -1616,7 +1790,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		break;
 	}
...
@@ -1679,7 +1853,7 @@ sg_poll(struct file *filp, poll_table * wait)
...
+	else if (likely(test_bit(SG_FFD_CMD_Q, sfp->ffd_bm)))
 		...
 		p_res |= EPOLLOUT | EPOLLWRNORM;
@@ -1701,9 +1875,10 @@ static vm_fault_t
...
@@ -1722,12 +1897,18 @@ sg_vma_fault(struct vm_fault *vmf)
...
@@ -1740,6 +1921,7 @@ sg_vma_fault(struct vm_fault *vmf)
 			struct page *pp;
...
+			xa_unlock_irqrestore(&sfp->srp_arr, iflags);
 			...
 			vmf->page = page;
@@ -1748,6 +1930,8 @@ sg_vma_fault(struct vm_fault *vmf)
 		sa += len;
 		offset -= len;
 	}
+out_err_unlock:
...
@@ -1762,9 +1946,10 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 {
 	int k, length;
 	int ret = 0;
...
+	struct sg_request *srp;
 	...
 	if (!filp || !vma)
 		return -ENXIO;
@@ -1780,11 +1965,13 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	rsv_schp = &srp->sgat_h;
 	if (req_sz > (unsigned long)rsv_schp->buflen) {
 		ret = -ENOMEM;
 		goto out;
@@ -1797,15 +1984,23 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 		sa += len;
 	}
...
+ */
 ...
 {
@@ -1823,8 +2018,10 @@ sg_rq_end_io_usercontext(struct work_struct *work)
 		return;
 	}
 	SG_LOG(3, sfp, "%s: srp=0x%pK\n", __func__, srp);
-	sg_finish_scsi_blk_rq(srp);
...
@@ -1862,85 +2059,100 @@ sg_check_sense(struct sg_device *sdp, struct sg_request *srp, int sense_len)
 }
...
@@ -1968,9 +2180,9 @@ static struct sg_device *
...
+	unsigned long iflags;
 	...
 	if (!sdp)
@@ -1988,7 +2200,7 @@ sg_add_device_helper(struct gendisk *disk, struct scsi_device *scsidp)
 			error = -ENODEV;
 		} else {
...
@@ -2244,31 +2471,40 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	q = sdp->device->request_queue;
 	...
 	/*
 	 * NOTE
@@ -2281,132 +2517,156 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
@@ -2418,6 +2678,7 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
+	struct sg_scatter_hold *schp = &srp->sgat_h;
 	...
 	if (unlikely(m_size < 0))
 		return -EFAULT;
@@ -2427,9 +2688,8 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
@@ -2493,13 +2753,15 @@ sg_remove_sgat_helper(struct sg_fd *sfp, struct sg_scatter_hold *schp)
...
@@ -2514,7 +2776,7 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 {
 	int k, num, res;
 	struct page *pgp;
...
@@ -2528,11 +2790,11 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 			break;
 		}
 		if (num > num_xfer) {
...
@@ -2555,135 +2817,274 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 static struct sg_request *
...
+ */
 ...
 {
@@ -2691,34 +3092,43 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
...
@@ -2732,28 +3142,64 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2784,12 +3230,14 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	struct sg_request *srp;
...
@@ -2797,24 +3245,20 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 			continue;
...
+	xa_unlock_irqrestore(xadp, iflags);
 	if (unlikely(sfp != e_sfp))
 		SG_LOG(1, sfp, "%s: xa_erase() return unexpected\n",
 		       __func__);
@@ -3066,6 +3510,7 @@ sg_proc_seq_show_devhdr(struct seq_file *s, void *v)
...
+	int fd_index;
 ...
 static void *
@@ -3149,11 +3594,10 @@ sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
 static void
...
@@ -3164,51 +3608,53 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 		k++;
