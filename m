...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 88D291388D8
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:10 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387506AbgALX6J (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:09 -0500
Received: from smtp.infotech.no ([82.134.31.41]:51950 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387498AbgALX6J (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:09 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id EBF47204259;
        Mon, 13 Jan 2020 00:58:05 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id nX7-8kHGGZds; Mon, 13 Jan 2020 00:58:02 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id D86B4204258;
        Mon, 13 Jan 2020 00:57:56 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Hannes Reinecke <hare@suse.com>
Subject: [PATCH v6 03/37] sg: sg_log and is_enabled
Date:   Mon, 13 Jan 2020 00:57:21 +0100
Message-Id: <20200112235755.14197-4-dgilbert@interlog.com>
...
index 62c5f58ff567..c8acd424bfd7 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -449,8 +489,7 @@ sg_write(struct file *filp, const char __user *buf, size_t count, loff_t * ppos)
 		return -EFAULT;
...
+	       (unsigned int)opcode, cmd_size);
 	...
 	mxsize -= SZ_SG_HEADER;
@@ -544,8 +582,7 @@ sg_new_write(struct sg_fd *sfp, struct file *file, const char __user *buf,
...
@@ -610,17 +647,15 @@ sg_common_write(struct sg_fd *sfp, struct sg_request *srp,
 	hp->host_status = 0;
 	hp->driver_status = 0;
 	hp->resid = 0;
...
@@ -753,9 +788,7 @@ sg_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
...
@@ -977,10 +1010,10 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
-				   "sg_ioctl: cmd=0x%x\n", (int) cmd_in));
 	...
 	switch (cmd_in) {
@@ -1204,7 +1237,7 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
...
+#if IS_ENABLED(CONFIG_COMPAT)
 ...
 {
@@ -1263,24 +1296,16 @@ sg_poll(struct file *filp, poll_table * wait)
...
@@ -1307,10 +1332,8 @@ sg_vma_fault(struct vm_fault *vmf)
...
+	SG_LOG(3, sfp, "%s: vm_start=0x%lx, off=%lu\n", __func__, sa, offset);
 	...
 	for (k = 0; k < rsv_schp->k_use_sg && sa < vma->vm_end; k++) {
 		len = vma->vm_end - sa;
@@ -1350,9 +1373,8 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 		return -ENXIO;
 	}
...
@@ -1421,10 +1443,9 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
 	result = req->result;
 	resid = req->resid_len;
...
@@ -1498,7 +1519,7 @@ static const struct file_operations sg_fops = {
 	.write = sg_write,
 	.poll = sg_poll,
...
+#if IS_ENABLED(CONFIG_COMPAT)
 ...
 #endif
 	.open = sg_open,
@@ -1545,7 +1566,7 @@ sg_alloc(struct gendisk *disk, struct scsi_device *scsidp)
 	k = error;
...
+			 "%s: dev=%d, sdp=0x%p ++\n", __func__, k, sdp));
 	...
 	disk->first_minor = k;
 	sdp->disk = disk;
@@ -1664,8 +1685,8 @@ sg_device_destroy(struct kref *kref)
...
+		    (current ? current->pid : -1), __func__, sdp);
 	...
 	put_disk(sdp->disk);
 	kfree(sdp);
@@ -1687,8 +1708,8 @@ sg_remove_device(struct device *cl_dev, struct class_interface *cl_intf)
 	if (val > 1)
...
+#if !IS_ENABLED(CONFIG_SCSI_PROC_FS)
 ...
 sg_proc_init(void)
 {
@@ -1768,9 +1789,8 @@ sg_proc_init(void)
 static void __exit
 exit_sg(void)
 {
...
+		remove_proc_subtree("scsi/sg", NULL);
 	...
 	sg_sysfs_valid = 0;
@@ -1797,15 +1817,14 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
+	       (rw ? "OUT" : "IN"));
 	...
 	/*
 	 * NOTE
@@ -1924,9 +1943,8 @@ sg_finish_rem_req(struct sg_request *srp)
...
@@ -1973,9 +1991,8 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
...
@@ -2014,18 +2031,13 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
...
+	SG_LOG(5, sfp, "%s: k_use_sg=%d, order=%d\n", __func__, k, order);
 	...
 	if (rem_sz > 0)	/* must have failed */
 		return -ENOMEM;
@@ -2043,35 +2055,34 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 static void
...
@@ -2101,8 +2112,7 @@ sg_build_reserve(struct sg_fd *sfp, int req_size)
 {
...
+	SG_LOG(3, sfp, "%s: buflen=%d\n", __func__, req_size);
 	do {
 		if (req_size < PAGE_SIZE)
 			req_size = PAGE_SIZE;
@@ -2122,8 +2132,7 @@ sg_link_reserve(struct sg_fd *sfp, struct sg_request *srp, int size)
 	int k, num, rem;
...
@@ -2141,8 +2150,7 @@ sg_link_reserve(struct sg_fd *sfp, struct sg_request *srp, int size)
 	}
...
@@ -2151,9 +2159,8 @@ sg_unlink_reserve(struct sg_fd *sfp, struct sg_request *srp)
 {
 	struct sg_scatter_hold *req_schp = &srp->data;
...
+	       (int)req_schp->k_use_sg);
 	...
 	req_schp->bufflen = 0;
 	req_schp->pages = NULL;
@@ -2244,18 +2251,15 @@ sg_add_sfp(struct sg_device *sdp)
 	}
...
@@ -2281,15 +2285,12 @@ sg_remove_sfp_usercontext(struct work_struct *work)
...
@@ -2312,7 +2313,6 @@ sg_remove_sfp(struct kref *kref)
...
-#ifdef CONFIG_SCSI_PROC_FS
 ...
 {
@@ -2324,19 +2324,6 @@ sg_idr_max_id(int id, void *p, void *data)
 	return 0;
 }
...
@@ -2366,7 +2353,7 @@ sg_get_dev(int dev)
 	return sdp;
 }
...
@@ -2441,6 +2428,17 @@ sg_proc_init(void)
 	return 0;
 }
...
@@ -2699,7 +2697,7 @@ sg_proc_seq_show_debug(struct seq_file *s, void *v)
 	return 0;
 }
