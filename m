...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 98E564202D2
	for <lists+linux-scsi@lfdr.de>; Sun,  3 Oct 2021 18:37:52 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231311AbhJCQjj (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 3 Oct 2021 12:39:39 -0400
Received: from smtp.infotech.no ([82.134.31.41]:55284 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231318AbhJCQji (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 3 Oct 2021 12:39:38 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id BF7062041DC;
        Sun,  3 Oct 2021 18:37:48 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id wipxI4YUUBkD; Sun,  3 Oct 2021 18:37:46 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-91-187-47.dyn.295.ca [23.91.187.47])
        by smtp.infotech.no (Postfix) with ESMTPA id 6F55920423B;
        Sun,  3 Oct 2021 18:36:35 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Damien.LeMoal@wdc.com
Subject: [PATCH v21 27/46] sg: add sg v4 interface support
Date:   Sun,  3 Oct 2021 12:31:32 -0400
Message-Id: <20211003163151.585349-28-dgilbert@interlog.com>
...
@@ -245,13 +265,13 @@ static void sg_rq_end_io(struct request *rq, blk_status_t status);
...
+		res = sg_v3_submit(filp, sfp, h3p, false, NULL);
 ...
 to_v2:
@@ -681,7 +711,7 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
 static inline int
...
@@ -713,8 +743,8 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 }
...
@@ -748,6 +778,67 @@ sg_submit(struct file *filp, struct sg_fd *sfp, struct sg_io_hdr *hp,
 	return 0;
 }
...
@@ -857,16 +948,45 @@ sg_rq_state_chg(struct sg_request *srp, enum sg_rq_state old_st,
 	return 0;
 }
...
@@ -903,24 +1053,7 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
 		goto err_out;
 	}
...
@@ -932,7 +1065,6 @@ sg_common_write(struct sg_fd *sfp, struct sg_comm_wr_t *cwrp)
...
-
 ...
 {
@@ -952,7 +1084,7 @@ sg_get_ready_srp(struct sg_fd *sfp, struct sg_request **srpp, int pack_id)
  * negated errno value.
  */
 static int
...
+sg_copy_sense(struct sg_request *srp, bool v4_active)
 {
 	int sb_len_ret = 0;
 	int scsi_stat;
@@ -962,11 +1094,18 @@ sg_copy_sense(struct sg_request *srp)
...
@@ -983,14 +1122,16 @@ sg_copy_sense(struct sg_request *srp)
 }
...
@@ -1017,7 +1158,7 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 		goto err_out;
 	}
...
@@ -1041,11 +1182,103 @@ sg_receive_v3(struct sg_fd *sfp, struct sg_request *srp, size_t count,
 	return err;
 }
...
+
 ...
 sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	     struct sg_request *srp)
@@ -1323,6 +1556,8 @@ sg_fill_request_element(struct sg_fd *sfp, struct sg_request *srp,
...
@@ -1340,7 +1575,7 @@ sg_rq_landed(struct sg_device *sdp, struct sg_request *srp)
  */
 static int
...
@@ -1368,7 +1603,10 @@ sg_wait_event_srp(struct file *filp, struct sg_fd *sfp, void __user *p,
...
@@ -1382,8 +1620,9 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 {
 	int res;
...
@@ -1392,15 +1631,25 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return res;
...
+	res = sg_wait_event_srp(filp, sfp, p, h4p, srp);
 	if (res)
 		SG_LOG(1, sfp, "%s: %s=0x%pK  state: %s\n", __func__,
 		       "unexpected srp", srp,
@@ -1569,6 +1818,12 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	switch (cmd_in) {
 	case SG_IO:
...
@@ -2026,8 +2281,16 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	}
 	...
 	SG_LOG(6, sfp, "%s: pack_id=%d, res=0x%x\n", __func__, srp->pack_id,
 	       srp->rq_result);
@@ -2417,7 +2680,8 @@ sg_set_map_data(const struct sg_scatter_hold *schp, bool up_valid,
 }
...
+	     struct sg_io_v4 *h4p, int dxfer_dir)
 {
 	...
 	int res = 0;
@@ -2434,7 +2698,6 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
...
-	struct sg_slice_hdr3 *sh3p = &srp->s_hdr3;
 	...
 	sdp = sfp->parentdp;
@@ -2444,10 +2707,28 @@ sg_start_req(struct sg_request *srp, u8 *cmd, int cmd_len, int dxfer_dir)
 			return -ENOMEM;
...
index 5213d79ecc84..489f8f7dfca5 100644
...
@@ -168,7 +180,6 @@ typedef struct sg_req_info {	/* used by SG_GET_REQUEST_TABLE ioctl() */
...
@@ -197,6 +208,13 @@ typedef struct sg_req_info {	/* used by SG_GET_REQUEST_TABLE ioctl() */
