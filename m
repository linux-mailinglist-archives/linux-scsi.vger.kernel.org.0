...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 29E1F38DE96
	for <lists+linux-scsi@lfdr.de>; Mon, 24 May 2021 03:02:06 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S232143AbhEXBDa (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 23 May 2021 21:03:30 -0400
Received: from smtp.infotech.no ([82.134.31.41]:33093 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S232072AbhEXBD2 (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 23 May 2021 21:03:28 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 290D8204278;
        Mon, 24 May 2021 03:02:00 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id YGmK8xwfRL8i; Mon, 24 May 2021 03:01:56 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-45-58-219-4.dyn.295.ca [45.58.219.4])
        by smtp.infotech.no (Postfix) with ESMTPA id 24857204269;
        Mon, 24 May 2021 03:01:52 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Johannes Thumshirn <Johannes.Thumshirn@wdc.com>,
        Christoph Hellwig <hch@lst.de>, Hannes Reinecke <hare@suse.com>
Subject: [PATCH v19 02/45] sg: remove typedefs, type+formatting cleanup
Date:   Sun, 23 May 2021 21:01:04 -0400
Message-Id: <20210524010147.94845-3-dgilbert@interlog.com>
...
index c7bc15c876df..e26284512544 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
@@ -3,7 +3,7 @@
  *  History:
...
@@ -1891,12 +1920,12 @@ sg_start_req(Sg_request *srp, unsigned char *cmd)
 }
...
@@ -1918,7 +1947,8 @@ sg_finish_rem_req(Sg_request *srp)
 }
...
@@ -1931,7 +1961,8 @@ sg_build_sgat(Sg_scatter_hold * schp, const Sg_fd * sfp, int tablesize)
 }
...
@@ -2009,7 +2040,7 @@ sg_build_indirect(Sg_scatter_hold * schp, Sg_fd * sfp, int buff_size)
 }
...
@@ -2032,9 +2063,9 @@ sg_remove_scat(Sg_fd * sfp, Sg_scatter_hold * schp)
 }
...
@@ -2065,9 +2096,9 @@ sg_read_oxfer(Sg_request * srp, char __user *outp, int num_read_xfer)
 }
...
@@ -2083,10 +2114,10 @@ sg_build_reserve(Sg_fd * sfp, int req_size)
 }
...
+	struct sg_scatter_hold *rsv_schp = &sfp->reserve;
 	...
 	srp->res_used = 1;
@@ -2115,9 +2146,9 @@ sg_link_reserve(Sg_fd * sfp, Sg_request * srp, int size)
 }
...
@@ -2133,12 +2164,12 @@ sg_unlink_reserve(Sg_fd * sfp, Sg_request * srp)
 }
...
@@ -2152,7 +2183,7 @@ sg_add_request(Sg_fd * sfp)
 		if (k >= SG_MAX_QUEUE)
 			goto out_unlock;
 	}
...
+sg_remove_request(struct sg_fd *sfp, struct sg_request *srp)
 {
 	unsigned long iflags;
 	int res = 0;
@@ -2182,10 +2213,10 @@ sg_remove_request(Sg_fd * sfp, Sg_request * srp)
 	return res;
 }
...
@@ -2235,13 +2266,13 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 {
...
+		srp = list_first_entry(&sfp->rq_list, struct sg_request, entry);
 		...
 		list_del(&srp->entry);
 		srp->parentfp = NULL;
@@ -2306,12 +2337,13 @@ sg_last_dev(void)
 #endif
...
+static struct sg_device *
 ...
 {
 	struct sg_device *sdp;
@@ -2407,13 +2439,15 @@ sg_proc_init(void)
 }
...
@@ -2434,7 +2468,8 @@ sg_proc_write_adio(struct file *filp, const char __user *buffer,
 	return count;
 }
...
@@ -2459,14 +2494,16 @@ sg_proc_write_dressz(struct file *filp, const char __user *buffer,
 	return -ERANGE;
 }
...
+sg_proc_seq_show_devhdr(struct seq_file *s, void *v)
 {
 	...
 	return 0;
@@ -2477,7 +2514,8 @@ struct sg_proc_deviter {
 	size_t	max;
 };
...
@@ -2492,7 +2530,8 @@ static void * dev_seq_start(struct seq_file *s, loff_t *pos)
 	return it;
 }
...
@@ -2500,15 +2539,17 @@ static void * dev_seq_next(struct seq_file *s, void *v, loff_t *pos)
...
@@ -2531,10 +2572,11 @@ static int sg_proc_seq_show_dev(struct seq_file *s, void *v)
 	return 0;
 }
...
@@ -2551,11 +2593,12 @@ static int sg_proc_seq_show_devstrs(struct seq_file *s, void *v)
 }
...
+	struct sg_fd *fp;
 	...
 	const char * cp;
 	unsigned int ms;
@@ -2613,10 +2656,11 @@ static void sg_proc_debug_helper(struct seq_file *s, Sg_device * sdp)
 	}
 }
