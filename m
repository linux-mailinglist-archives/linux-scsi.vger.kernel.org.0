...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 815351388E8
	for <lists+linux-scsi@lfdr.de>; Mon, 13 Jan 2020 00:58:27 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2387544AbgALX61 (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 12 Jan 2020 18:58:27 -0500
Received: from smtp.infotech.no ([82.134.31.41]:52031 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S2387511AbgALX60 (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 12 Jan 2020 18:58:26 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id E102A20415B;
        Mon, 13 Jan 2020 00:58:24 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id 1f6-UwV-NcP6; Mon, 13 Jan 2020 00:58:22 +0100 (CET)
Received: from xtwo70.bingwo.ca (unknown [213.52.86.138])
        by smtp.infotech.no (Postfix) with ESMTPA id A182E20429F;
        Mon, 13 Jan 2020 00:57:59 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v6 20/37] sg: sg_find_srp_by_id
Date:   Mon, 13 Jan 2020 00:57:38 +0100
Message-Id: <20200112235755.14197-21-dgilbert@interlog.com>
...
---
 drivers/scsi/sg.c | 292 +++++++++++++++++++++++++++++++---------------
 1 file changed, 199 insertions(+), 93 deletions(-)

diff --git a/drivers/scsi/sg.c b/drivers/scsi/sg.c
index 7eb77c2247ca..275b5d800196 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
@@ -155,16 +155,19 @@ struct sg_fd;
...
@@ -230,7 +233,7 @@ static ssize_t sg_submit(struct sg_fd *sfp, struct file *filp,
...
@@ -240,6 +243,7 @@ static void sg_link_reserve(struct sg_fd *sfp, struct sg_request *srp,
...
@@ -451,7 +455,8 @@ sg_open(struct inode *inode, struct file *filp)
...
@@ -520,7 +525,6 @@ sg_release(struct inode *inode, struct file *filp)
 static ssize_t
...
@@ -619,21 +623,19 @@ sg_write(struct file *filp, const char __user *p, size_t count, loff_t *ppos)
 		return -EFAULT;
 	/*
...
@@ -661,6 +663,18 @@ sg_fetch_cmnd(struct file *filp, struct sg_fd *sfp, const u8 __user *u_cdbp,
 	return 0;
 }
...
@@ -693,17 +707,10 @@ sg_submit(struct sg_fd *sfp, struct file *filp, const char __user *buf,
 		return -ENOSYS;
 	}
...
@@ -725,6 +732,12 @@ sg_submit(struct sg_fd *sfp, struct file *filp, const char __user *buf,
 	return count;
 }
...
+ */
 ...
 {
@@ -803,37 +816,45 @@ sg_rstate_chg(struct sg_request *srp, enum sg_rq_state old_st,
...
@@ -862,37 +883,32 @@ sg_rep_rq_state_fail(struct sg_fd *sfp, enum sg_rq_state exp_old_st,
 }
 #endif
...
-err_out:
 	...
 	sg_deact_request(sfp, srp);
 	return err ? err : 0;
@@ -971,16 +987,22 @@ sg_read_v1v2(void __user *buf, int count, struct sg_fd *sfp,
 	return res;
 }
...
@@ -995,7 +1017,7 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1014,17 +1036,13 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
...
@@ -1032,27 +1050,27 @@ sg_read(struct file *filp, char __user *p, size_t count, loff_t *ppos)
 			want_id = h2p->pack_id;
 		}
 	}
...
@@ -1102,6 +1120,52 @@ sg_calc_sgat_param(struct sg_device *sdp)
 	sdp->max_sgat_sz = sz;
 }
...
+
 ...
 {
@@ -1118,6 +1182,7 @@ sg_fill_request_table(struct sg_fd *sfp, struct sg_req_info *rinfo)
...
+		rinfo[val].duration = sg_get_dur(srp, NULL, NULL); /* dummy */
 		if (srp->done)
 			rinfo[val].duration =
 				srp->header.duration;
@@ -1135,6 +1200,18 @@ sg_fill_request_table(struct sg_fd *sfp, struct sg_req_info *rinfo)
 	}
 }
...
@@ -1162,7 +1239,7 @@ sg_ctl_sg_io(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	if (srp->done) {
 		srp->done = 2;
...
+		res = sg_receive_v3(sfp, srp, p);
 	...
 	srp->orphan = 1;
@@ -1397,7 +1474,9 @@ sg_ioctl(struct file *filp, unsigned int cmd_in, unsigned long arg)
 			return result;
...
@@ -2349,6 +2428,33 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
 	return res;
 }
