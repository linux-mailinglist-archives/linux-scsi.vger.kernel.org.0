...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id C016C4202B5
	for <lists+linux-scsi@lfdr.de>; Sun,  3 Oct 2021 18:33:26 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231233AbhJCQfN (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 3 Oct 2021 12:35:13 -0400
Received: from smtp.infotech.no ([82.134.31.41]:55086 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231132AbhJCQfM (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 3 Oct 2021 12:35:12 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 88DC42041BD;
        Sun,  3 Oct 2021 18:32:40 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id GkI8mEV5pI99; Sun,  3 Oct 2021 18:32:33 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-91-187-47.dyn.295.ca [23.91.187.47])
        by smtp.infotech.no (Postfix) with ESMTPA id 254502041BB;
        Sun,  3 Oct 2021 18:31:58 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Damien.LeMoal@wdc.com, Hannes Reinecke <hare@suse.com>
Subject: [PATCH v21 03/46] sg: sg_log and is_enabled
Date:   Sun,  3 Oct 2021 12:31:08 -0400
Message-Id: <20211003163151.585349-4-dgilbert@interlog.com>
...
index bc6d372cb4c7..4c8070467b5d 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
@@ -753,9 +789,7 @@ sg_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
...
@@ -978,8 +1012,8 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
...
+	       !!(filp->f_flags & O_NONBLOCK));
 	...
 	switch (cmd_in) {
@@ -1255,24 +1289,16 @@ sg_poll(struct file *filp, poll_table * wait)
...
@@ -1299,10 +1325,8 @@ sg_vma_fault(struct vm_fault *vmf)
...
+	SG_LOG(3, sfp, "%s: vm_start=0x%lx, off=%lu\n", __func__, sa, offset);
 	...
 	for (k = 0; k < rsv_schp->k_use_sg && sa < vma->vm_end; k++) {
 		len = vma->vm_end - sa;
@@ -1342,9 +1366,8 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
 		return -ENXIO;
 	}
...
@@ -1413,10 +1436,9 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
 	result = req->result;
 	resid = req->resid_len;
...
@@ -1535,7 +1557,7 @@ sg_alloc(struct scsi_device *scsidp)
 	k = error;
...
@@ -1634,7 +1656,11 @@ sg_device_destroy(struct kref *kref)
...
+	 * CAUTION!  Note that the device can still be found via idr_find()
 	 ...
 	 * any other cleanup.
 	 */
@@ -1643,9 +1669,6 @@ sg_device_destroy(struct kref *kref)
...
@@ -1665,8 +1688,8 @@ sg_remove_device(struct device *cl_dev, struct class_interface *cl_intf)
 	if (val > 1)
...
+#if !IS_ENABLED(CONFIG_SCSI_PROC_FS)
 ...
 sg_proc_init(void)
 {
@@ -1746,9 +1769,8 @@ sg_proc_init(void)
 static void __exit
 exit_sg(void)
 {
...
+		remove_proc_subtree("scsi/sg", NULL);
 	...
 	sg_sysfs_valid = 0;
@@ -1775,15 +1797,14 @@ sg_start_req(struct sg_request *srp, u8 *cmd)
...
+	       (rw ? "OUT" : "IN"));
 	...
 	/*
 	 * NOTE
@@ -1894,9 +1915,8 @@ sg_finish_rem_req(struct sg_request *srp)
...
@@ -1942,9 +1962,8 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
...
@@ -1980,18 +1999,13 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
...
+	SG_LOG(5, sfp, "%s: k_use_sg=%d, order=%d\n", __func__, k, order);
 	...
 	if (rem_sz > 0)	/* must have failed */
 		return -ENOMEM;
@@ -2009,35 +2023,34 @@ sg_build_indirect(struct sg_scatter_hold *schp, struct sg_fd *sfp,
 static void
...
@@ -2067,8 +2080,7 @@ sg_build_reserve(struct sg_fd *sfp, int req_size)
 {
...
+	SG_LOG(3, sfp, "%s: buflen=%d\n", __func__, req_size);
 	do {
 		if (req_size < PAGE_SIZE)
 			req_size = PAGE_SIZE;
@@ -2088,8 +2100,7 @@ sg_link_reserve(struct sg_fd *sfp, struct sg_request *srp, int size)
 	int k, num, rem;
...
@@ -2107,8 +2118,7 @@ sg_link_reserve(struct sg_fd *sfp, struct sg_request *srp, int size)
 	}
...
@@ -2117,9 +2127,8 @@ sg_unlink_reserve(struct sg_fd *sfp, struct sg_request *srp)
 {
 	struct sg_scatter_hold *req_schp = &srp->data;
...
+	       (int)req_schp->k_use_sg);
 	...
 	req_schp->bufflen = 0;
 	req_schp->pages = NULL;
@@ -2210,18 +2219,15 @@ sg_add_sfp(struct sg_device *sdp)
 	}
...
@@ -2247,15 +2253,12 @@ sg_remove_sfp_usercontext(struct work_struct *work)
...
@@ -2278,7 +2281,6 @@ sg_remove_sfp(struct kref *kref)
...
-#ifdef CONFIG_SCSI_PROC_FS
 ...
 {
@@ -2290,19 +2292,6 @@ sg_idr_max_id(int id, void *p, void *data)
 	return 0;
 }
...
@@ -2332,7 +2321,7 @@ sg_get_dev(int dev)
 	return sdp;
 }
...
@@ -2405,6 +2394,17 @@ sg_proc_init(void)
 	return 0;
 }
...
@@ -2662,7 +2662,7 @@ sg_proc_seq_show_debug(struct seq_file *s, void *v)
 	return 0;
 }
