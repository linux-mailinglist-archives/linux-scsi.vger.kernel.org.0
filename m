...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 753B44202DB
	for <lists+linux-scsi@lfdr.de>; Sun,  3 Oct 2021 18:38:18 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231261AbhJCQkD (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Sun, 3 Oct 2021 12:40:03 -0400
Received: from smtp.infotech.no ([82.134.31.41]:55314 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231246AbhJCQjw (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Sun, 3 Oct 2021 12:39:52 -0400
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id E6A39204155;
        Sun,  3 Oct 2021 18:38:00 +0200 (CEST)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id WAsomAPUK5sk; Sun,  3 Oct 2021 18:37:58 +0200 (CEST)
Received: from xtwo70.bingwo.ca (host-23-91-187-47.dyn.295.ca [23.91.187.47])
        by smtp.infotech.no (Postfix) with ESMTPA id 016162041CB;
        Sun,  3 Oct 2021 18:37:32 +0200 (CEST)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de,
        Damien.LeMoal@wdc.com
Subject: [PATCH v21 36/46] sg: rework mmap support
Date:   Sun,  3 Oct 2021 12:31:41 -0400
Message-Id: <20211003163151.585349-37-dgilbert@interlog.com>
...
index d49287b6c2c8..62dc02ef12e7 100644
--- a/drivers/scsi/sg.c
+++ b/drivers/scsi/sg.c
...
+	WRITE_ONCE(cwr.frq_bm[0], 0);
 	...
 	cwr.filp = filp;
@@ -770,7 +771,7 @@ sg_v3_submit(struct file *filp, struct sg_fd *sfp, struct sg_io_hdr *hp,
...
@@ -811,7 +812,7 @@ sg_submit_v4(struct file *filp, struct sg_fd *sfp, void __user *p,
...
+	WRITE_ONCE(cwr.frq_bm[0], 0);
 	...
 	cwr.h4p = h4p;
@@ -1762,6 +1763,8 @@ sg_set_reserved_sz(struct sg_fd *sfp, int want_rsv_sz)
 	srp = sfp->rsv_srp;
 	if (!srp)
 		return -EPROTO;
...
@@ -2153,13 +2156,39 @@ sg_fasync(int fd, struct file *filp, int mode)
...
@@ -2185,7 +2214,7 @@ sg_vma_fault(struct vm_fault *vmf)
...
@@ -2193,44 +2222,37 @@ sg_vma_fault(struct vm_fault *vmf)
 		       offset);
 		goto out_err_unlock;
 	}
-	sa = vma->vm_start;
...
@@ -2241,40 +2263,48 @@ sg_mmap(struct file *filp, struct vm_area_struct *vma)
...
+	return res;
 ...
 static void
@@ -2839,7 +2869,7 @@ sg_start_req(struct sg_request *srp, struct sg_comm_wr_t *cwrp, int dxfer_dir)
 		goto fini;
...
+	us_xfer = !(srp->rq_flags & (SG_FLAG_NO_DXFER | SG_FLAG_MMAP_IO));
 	...
 	rq->end_io_data = srp;
@@ -2968,10 +2998,11 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
 {
...
+	struct page **pgp;
 	...
 	if (unlikely(minlen <= 0)) {
 		if (minlen < 0)
@@ -2986,30 +3017,35 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
...
@@ -3017,6 +3053,7 @@ sg_mk_sgat(struct sg_scatter_hold *schp, struct sg_fd *sfp, int minlen)
 		elem_sz >>= 1;
 		goto again;
 	}
+b4_alloc_pages:
 	...
 	schp->pages = NULL;
 	return -ENOMEM;
@@ -3032,7 +3069,7 @@ sg_remove_sgat_helper(struct sg_fd *sfp, struct sg_scatter_hold *schp)
 		return;
...
@@ -3162,7 +3199,7 @@ sg_find_srp_by_id(struct sg_fd *sfp, int pack_id)
 	}
 	return NULL;
 good:
...
@@ -3358,7 +3395,7 @@ sg_setup_req(struct sg_comm_wr_t *cwrp, int dxfr_len)
 		r_srp->parentfp = fp;
...
+	WRITE_ONCE(r_srp->frq_bm[0], cwrp->frq_bm[0]);	/* assumes <= 32 req flags */
 ...
 fini:
@@ -3385,7 +3422,7 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
 		return;
 	sbp = srp->sense_bp;
 	srp->sense_bp = NULL;
-	srp->frq_bm[0] = 0;
