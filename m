...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 84006172441
	for <lists+linux-scsi@lfdr.de>; Thu, 27 Feb 2020 17:59:52 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729297AbgB0Q7s (ORCPT <rfc822;lists+linux-scsi@lfdr.de>);
        Thu, 27 Feb 2020 11:59:48 -0500
Received: from smtp.infotech.no ([82.134.31.41]:47251 "EHLO smtp.infotech.no"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1729768AbgB0Q7s (ORCPT <rfc822;linux-scsi@vger.kernel.org>);
        Thu, 27 Feb 2020 11:59:48 -0500
Received: from localhost (localhost [127.0.0.1])
        by smtp.infotech.no (Postfix) with ESMTP id 44A6A20418F;
        Thu, 27 Feb 2020 17:59:45 +0100 (CET)
X-Virus-Scanned: by amavisd-new-2.6.6 (20110518) (Debian) at infotech.no
Received: from smtp.infotech.no ([127.0.0.1])
        by localhost (smtp.infotech.no [127.0.0.1]) (amavisd-new, port 10024)
        with ESMTP id zQnl98HNEOpR; Thu, 27 Feb 2020 17:59:42 +0100 (CET)
Received: from xtwo70.bingwo.ca (host-23-251-188-50.dyn.295.ca [23.251.188.50])
        by smtp.infotech.no (Postfix) with ESMTPA id 8ACF2204191;
        Thu, 27 Feb 2020 17:59:37 +0100 (CET)
From:   Douglas Gilbert <dgilbert@interlog.com>
To:     linux-scsi@vger.kernel.org
Cc:     martin.petersen@oracle.com, jejb@linux.vnet.ibm.com, hare@suse.de
Subject: [PATCH v7 24/38] sg: xarray for reqs in fd
Date:   Thu, 27 Feb 2020 11:58:48 -0500
Message-Id: <20200227165902.11861-25-dgilbert@interlog.com>
...
@@ -1308,7 +1433,7 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 	int __user *ip = p;
 	struct sg_request *srp;
...
@@ -1331,14 +1456,15 @@ sg_ioctl_common(struct file *filp, struct sg_device *sdp, struct sg_fd *sfp,
 		return 0;
...
@@ -1751,10 +1877,10 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
+	unsigned long iflags;
 	...
 	if (WARN_ON(srp->done != 0))
 		return;
@@ -1798,7 +1924,6 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
...
-	spin_lock_irqsave(&sfp->rq_list_lock, iflags);
 	if (unlikely(srp->orphan)) {
 		if (sfp->keep_orphan)
 			srp->sg_io_owned = 0;
@@ -1806,12 +1931,14 @@ sg_rq_end_io(struct request *rq, blk_status_t status)
 			done = 0;
 	}
 	srp->done = done;
-	spin_unlock_irqrestore(&sfp->rq_list_lock, iflags);
...
@@ -2433,20 +2560,19 @@ sg_read_append(struct sg_request *srp, void __user *outp, int num_xfer)
...
@@ -2516,31 +2642,51 @@ sg_build_reserve(struct sg_fd *sfp, int req_size)
 static struct sg_request *
...
-	return NULL;
 ...
 static void
@@ -2550,14 +2696,10 @@ sg_deact_request(struct sg_fd *sfp, struct sg_request *srp)
...
+	atomic_set(&srp->rq_st, SG_RS_INACTIVE);
 ...
 static struct sg_fd *
@@ -2574,8 +2716,7 @@ sg_add_sfp(struct sg_device *sdp)
...
@@ -2594,6 +2735,7 @@ sg_add_sfp(struct sg_device *sdp)
 	sfp->parentdp = sdp;
...
@@ -2653,15 +2797,20 @@ sg_remove_sfp_usercontext(struct work_struct *work)
 	sdp = sfp->parentdp;
...
@@ -2754,7 +2903,9 @@ sg_rq_st_str(enum sg_rq_state rq_st, bool long_str)
...
@@ -3016,7 +3167,6 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
 		if (!fp)
 			continue;
 		k++;
...
@@ -3027,7 +3177,9 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
...
+				continue;
 			...
 			if (srp->res_used) {
@@ -3063,9 +3215,8 @@ sg_proc_debug_helper(struct seq_file *s, struct sg_device *sdp)
